ALTER TABLE web_form MODIFY `custom_fields` TEXT(2000);ALTER TABLE structure ADD content TINYINT(1) default 1;
...
ALTER TABLE field ADD read_only TINYINT(1)  default 0;alter table tree add index (parent), add constraint FK36739EC4AB08AA foreign key (parent) references inode (inode);
alter table tree add index (child), add constraint FK36739E5A3F51C foreign key (child) references inode (inode);ALTER TABLE `field`  ADD COLUMN searchable TINYINT(1);
update `field` f set f.searchable = indexed;ALTER TABLE contentlet CHANGE COLUMN `ENABLED_WYSIWYG` `disabled_wysiwyg` VARCHAR(255) DEFAULT NULL;CREATE TABLE users_to_delete (
...
update qrtz_job_details set job_class_name='com.dotmarketing.quartz.job.PopBouncedMailThread' where job_class_name='com.dotmarketing.threads.PopBouncedMailThread';create table event_recurrence (
...
add foreign key (inode) references inode(inode);/*!40014 SET @OLD_UNIQUE_CHECKS=@@UNIQUE_CHECKS, UNIQUE_CHECKS=0 */;
...
ALTER TABLE `dlfilerank` 
	CHANGE filename filename varchar(100) CHARACTER SET utf8;
ALTER TABLE `dlfilerank` 
	ADD primary key(companyid,userid,repositoryid);
ALTER TABLE `dlfilerank` 
	DEFAULT CHARACTER SET utf8;
