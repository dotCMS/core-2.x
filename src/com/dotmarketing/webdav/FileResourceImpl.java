...
public class FileResourceImpl implements FileResource, LockableResource {
	...
	public FileResourceImpl(File file, String path) {
	...
	public boolean authorise(Request req, Method method, Auth auth) {
		try {
			...
			if(auth == null)
				...
				return perAPI.doesUserHavePermission(file, PermissionAPI.PERMISSION_PUBLISH, user, false);
			}else if(method.isWrite && !isAutoPub){
				return perAPI.doesUserHavePermission(file, PermissionAPI.PERMISSION_EDIT, user, false);
			}else if(!method.isWrite){
				return perAPI.doesUserHavePermission(file, PermissionAPI.PERMISSION_READ, user, false);
			}
	...
	public Long getContentLength() {
		...
		try {
			workingFile = FileFactory.getAssetIOFile(file);
		} catch (IOException e) {
			Logger.error(this, e.getMessage(), e);
			return new Long(0);
		}
	...
	public String getContentType(String accepts) {
		return FileFactory.getMimeType(file.getFileName());
	}
	...
	public void sendContent(OutputStream out, Range arg1, Map<String, String> arg2, String arg3) throws IOException {
		java.io.File f;
		try {
			f = FileFactory.getAssetIOFile(file);
		} catch (IOException e) {
			Logger.error(this, e.getMessage(), e);
			return;
		}
