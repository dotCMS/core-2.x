...
public class DistributedJournalAPIImpl<T> implements DistributedJournalAPI<T> {
	...
	public void addContentIndexEntry(Contentlet content) throws DotDataException {
		if (!UtilMethods.isSet(content.getInode())) {
			Logger
					.warn(this,
							"You cannot add content to index whose inode is less then 1");
			return;
		}
		distFac.addContentIndexEntry(content);
	}
	...
	public boolean buildNewIndexForServer() throws DotDataException {
		return distFac.buildNewIndexForServer();
	}

	public void addContentIndexEntryToDelete(T contentIdentifier) throws DotDataException {
		distFac.addContentIndexEntryToDelete(contentIdentifier);
	}
