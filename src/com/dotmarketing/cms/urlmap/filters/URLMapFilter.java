	...
	public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain) throws IOException,
		...
		if (RegEX.contains(url, mastRegEx)) {
			...
			for (PatternCache pc : patternsCache) {
				...
				if (matches != null && matches.size() > 0) {
					...
					try {
						...
						Logger.warn(this, "No urlmap contentlent found uri:" + request.getRequestURI() + " query:" + query.toString());
					}
				}
			}
			...
			if (structure != null && UtilMethods.isSet(structure.getDetailPage())) {
				Identifier ident;
				try {
					ident = IdentifierCache.getIdentifierFromIdentifierCache(structure.getDetailPage());
