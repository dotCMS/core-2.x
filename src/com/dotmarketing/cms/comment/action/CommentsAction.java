...
public class CommentsAction extends DispatchAction {
	...
	public ActionForward saveComments(ActionMapping mapping, ActionForm lf, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			Identifier contentletIdentifier = IdentifierCache.getIdentifierFromIdentifierCache(contentlet);
			...
			Identifier contentletCommentIdentifier = IdentifierCache.getIdentifierFromIdentifierCache(contentletComment);
			...
			/**
			 * If the comments moderation field is set, a workflow task is created
			 */
			if(UtilMethods.isSet(commentsForm.getCommentsModeration())){
				...
				changeHist.append("Title: " + UtilHTML.escapeHTMLSpecialChars(commentsForm.getCommentTitle()) + "<br>");
				task.setTitle("A new comment titled: " + UtilHTML.escapeHTMLSpecialChars(commentsForm.getCommentTitle()) + " has been posted.");
				task.setDescription("A new comment titled " + UtilHTML.escapeHTMLSpecialChars(commentsForm.getCommentTitle()) + 
						" has been posted by " + UtilHTML.escapeHTMLSpecialChars(commentsForm.getName()) + " ("+commentsForm.getEmail()+")");
				changeHist.append("Description: " + UtilHTML.escapeHTMLSpecialChars(task.getDescription()) + "<br>");

				Role role = APILocator.getRoleAPI().loadRoleByKey(commentsForm.getCommentsModeration());
				if(role != null && UtilMethods.isSet(role.getId())){
					task.setBelongsTo(role.getId());
				}else{
					task.setBelongsTo(APILocator.getRoleAPI().loadCMSAdminRole().getId());
				}
				...
				task.setWebasset(contentletComment.getInode());

				InodeFactory.saveInode(task);

				//Save the comment
				WorkflowComment taskComment = new WorkflowComment ();
				taskComment.setComment(commentsForm.getComment());
				...
				WorkflowsFactory.sendWorkflowChangeEmails (task, "New user comment has been submitted", "New Task", null);        
			...
			InodeFactory.saveInode(beanContentlet);
			if (commentsForm.isCommentAutoPublish()) 
			{
	...
	private void sendCommentEmails(Identifier contentletIdentifier, String relationName, HttpServletRequest request, String referrer,String userName,String userEmail,String userComment,String commentQuestion) 
	{
		...
		//String condition = responseField + " = 'yes'";
		...
		List<Contentlet> comments = RelationshipFactory.getRelatedContentByParent(contentletIdentifier.getInode(), relationName, condition, order);
