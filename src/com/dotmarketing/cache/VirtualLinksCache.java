...
public class VirtualLinksCache {    
    ...
    public static String getPathFromCache(String url) {
    	...
    	try{
    		realLink = (String) cache.get(getPrimaryGroup() + url,getPrimaryGroup());
		...
		if(vl != null && InodeUtils.isSet(vl.getInode())) 
		{
            ...
            cache.put(getPrimaryGroup() + url, WebKeys.Cache.CACHE_NOT_FOUND, getPrimaryGroup());
            return null;
		}
    ...
    public static void addPathToCache(VirtualLink vl){
        ...
        if (vl != null && InodeUtils.isSet(vl.getInode())) 
        {
            ...
            cache.put(getPrimaryGroup() + vl.getUrl(), vl.getUri(), getPrimaryGroup());
        }
    }
    ...
    public static void removePathFromCache(String url) {
    	...
    	cache.remove(getPrimaryGroup() + url,getPrimaryGroup());
    } 
    
    public static void mapAllVirtualLinks() {
    	DotCacheAdministrator cache = CacheLocator.getCacheAdministrator();
        Iterator<VirtualLink> iter = VirtualLinkFactory.getVirtualLinks().iterator();
        while (iter.hasNext()) {
