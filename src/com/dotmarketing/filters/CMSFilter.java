...
public class CMSFilter implements Filter {
    ...
    public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain) throws IOException, ServletException {
    	...
    	//reindex thread lifechecker.  Call should return instantly so no worry about lag here
    	ReindexThread.ensureLife();
        ...
        if (uri.trim().equals("/c") || uri.trim().equals("/c/") ||
        ...
        if (PREVIEW_MODE || EDIT_MODE) {
            ...
            if (!UtilMethods.isSet(pointer) && (uri.endsWith(dotExtension) || InodeUtils.isSet(FolderFactory.getFolderByPath(uri, host).getInode()))) {
                String url = uri;
        ...
        if (UtilMethods.isSet(pointer)) {
            ...
            if (!endInTheVelocityPageExtension(pointer)) {
                ...
                try {
					...
					if (!permissionAPI.doesUserHavePermission(f, PermissionAPI.PERMISSION_READ, user)) {
...
//                                new ObjectValuePair(uri, request.getParameterMap()));
                ...
                String mimeType = FileFactory.getMimeType(Config.CONTEXT.getRealPath(pointer));
    ...
    public void init(FilterConfig config) throws ServletException {
        ...
        ASSET_PATH = FileFactory.getRelativeAssetsRootPath();
