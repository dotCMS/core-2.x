				...
				for(WebAsset webAsset : webAssets)
				{
					...
					else if(identifierInode != parentIdentifierInode)
					{
						...
						if(identifierInode != 0)
						{
							try
							{
								Identifier identifier = IdentifierCache.getIdentifierFromIdentifierCache(identifierInode);
						...
						{
							Identifier identifier = IdentifierCache.getIdentifierFromIdentifierCache(parentIdentifierInode);
							if(identifier.getInode() != 0)
							{
...
*/
	public static Map fixAssetsInconsistencies() throws Exception {
		...
		final String fix2ContainerQuery = "select c.* from containers c, inode i where i.inode = c.inode and i.identifier = ? order by live desc, mod_date desc";
		final String fix3ContainerQuery = "update containers set working = ? where inode = ?";

		final String fix2FileAssetQuery = "select c.* from file_asset c, inode i where i.inode = c.inode and i.identifier = ? order by live desc, mod_date desc";
		final String fix3FileAssetQuery = "update file_asset set working = ? where inode = ?";

		final String fix2HtmlPageQuery = "select c.* from htmlpage c, inode i where i.inode = c.inode and i.identifier = ? order by live desc, mod_date desc";
		final String fix3HtmlPageQuery = "update htmlpage set working = ? where inode = ?";

		final String fix2LinksQuery = "select c.* from links c, inode i where i.inode = c.inode and i.identifier = ? order by live desc, mod_date desc";
		final String fix3LinksQuery = "update links set working = ? where inode = ?";

		final String fix2TemplatesQuery = "select c.* from template c, inode i where i.inode = c.inode and i.identifier = ? order by live desc, mod_date desc";
