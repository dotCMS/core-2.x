...
public class ContentletAjax {
	 ...
	 */
	public List searchContentlet(String structureInode, List<String> fields, List<String> categories, boolean showDeleted, boolean filterSystemHost, int page, int perPage, String orderBy) {
		...
		return searchContentletsByUser(structureInode, fields, categories, showDeleted, filterSystemHost, page, orderBy, perPage, currentUser, sess, null, null);
	}
	
	public List searchContentlets(String structureInode, List<String> fields, List<String> categories, boolean showDeleted, boolean filterSystemHost, int page, String orderBy, String modDateFrom, String modDateTo) {
	...
	public List searchContentlets(String structureInode, List<String> fields, List<String> categories, boolean showDeleted, boolean filterSystemHost, int page, String orderBy, String modDateFrom, String modDateTo, boolean saveLastSearch) {
		...
		return searchContentletsByUser(structureInode, fields, categories, showDeleted, filterSystemHost, page, orderBy, 0,currentUser, sess, modDateFrom, modDateTo);
	}
	...
	public List searchContentletsByUser(String structureInode, List<String> fields, List<String> categories, boolean showDeleted, boolean filterSystemHost, int page, String orderBy,int perPage, User currentUser, HttpSession sess,String  modDateFrom, String modDateTo) {
		if(perPage < 1){
				...
				if(fieldName.equalsIgnoreCase("conhost")){
					if(!filterSystemHost && !fieldValue.equals(Host.SYSTEM_HOST)){
						...
						if( fieldbcontentname.startsWith("text") ){
							...
							if( FieldFactory.isTagField(fieldbcontentname,st)== false){
//								String quotes = fieldValue.contains(" ") ? "\"" : "";
								...
								luceneQuery.append("+" + st.getVelocityVarName() + "." + fieldVelocityVarName + ":" + (hasQuotes ? "\"":wildCard) + LuceneUtils.escape(fieldValue.toString()) + (hasQuotes ? "\"":wildCard) + " ");
							}
							else{
								...
								for(String splitValue : splitValues)
								{
									...
									luceneQuery.append("+" + st.getVelocityVarName() + "." + fieldVelocityVarName+ ":" + quotes + LuceneUtils.escape(splitValue.toString()) + quotes + " ");
								}
							}
						}
		...
		PaginatedArrayList <Contentlet> totalHits = new PaginatedArrayList <Contentlet>();
		try{
			hits = (PaginatedArrayList<Contentlet>) conAPI.search(luceneQuery.toString(), perPage + 1, offset, orderBy, currentUser, false);
			totalHits = (PaginatedArrayList<Contentlet>) conAPI.search(luceneQuery.toString(), 10000, 0, orderBy, currentUser, false);
		}catch (Exception pe) {
		...
		//Adding total search results inodes list as the third row of the results.
		StringBuffer allSearchedContentsInodes = new StringBuffer();
		for (int i = 0; i < totalHits.size(); ++i) {
			Contentlet contentlet;
			contentlet = totalHits.get(i);
			if(allSearchedContentsInodes.length() == 0)
				allSearchedContentsInodes.append(contentlet.getInode());
			else
				allSearchedContentsInodes.append(","+contentlet.getInode());		
		}
		results.add(allSearchedContentsInodes.toString());
	...
	public Map<String,Object> saveContent(List<String> formData, boolean isAutoSave,boolean isCheckin) throws LanguageException, PortalException, SystemException {	
		...
		for (Iterator iterator = formData.iterator(); iterator.hasNext();) {
			...
			if(elementName.startsWith("binary")){ 
				...
				if(UtilMethods.isSet(binaryFileValue) && !binaryFileValue.equals("---removed---")){
					while(binaryFileValue.indexOf(" ") > -1){
						binaryFileValue = binaryFileValue.replace(" ", "");
					}	
		...
		try {
			...
			{
				//contentlet
				File file = new File("c:\\contentlet.ser");
				if(!file.exists()) file.createNewFile();
				FileOutputStream fos = new FileOutputStream(file);
				ObjectOutputStream oos = new ObjectOutputStream(fos);
				oos.writeObject(contentletFormData);
				oos.close();
				//User
				file = new File("c:\\user.ser");
				if(!file.exists()) file.createNewFile();
			...
			if (contentlet.getStructure().getVelocityVarName().equalsIgnoreCase("host")) {
			...
			String urlMap = contentlet.getStructure().getUrlMapPattern();
