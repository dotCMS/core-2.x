...
public class OrderContentletAction  extends DotPortletAction {
	...
	throws Exception {
		...
		Identifier containerIdentifier = IdentifierFactory.getIdentifierByWebAsset(container);
		Identifier pageIdentifier = IdentifierFactory.getIdentifierByWebAsset(page);
		...
		for(Identifier ident : itemsIdentifierList){			
			List<com.dotmarketing.portlets.contentlet.business.Contentlet> fatConts =
				(List<com.dotmarketing.portlets.contentlet.business.Contentlet>) IdentifierFactory.getWorkingChildrenOfClass(ident, com.dotmarketing.portlets.contentlet.business.Contentlet.class);
		...
		try
		{
			...
			while(keysIterator.hasNext())
			{
				...
				for(int i = 0;i < hashInodes.size();i++)
				{
					...
					Identifier containerIdentifier = IdentifierFactory.getIdentifierByWebAsset(container);
					Identifier pageIdentifier = IdentifierFactory.getIdentifierByWebAsset(page);
					Identifier iden = IdentifierCache.getIdentifierFromIdentifierCache(c);
