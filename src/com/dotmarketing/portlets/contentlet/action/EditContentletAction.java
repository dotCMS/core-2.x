...
public class EditContentletAction extends DotPortletAction implements DotPortletActionInterface {
		...
		if ((cmd != null) && cmd.equals(Constants.ADD)) {
			try {
				...
				try {
					...
					if ((subcmd != null) && subcmd.equals(com.dotmarketing.util.Constants.ASSIGNTO)) {
						Logger.debug(this, "Calling AssignTo Method");
						_assignWebAsset(req, res, config, form, user, WebKeys.CONTENTLET_FORM_EDIT);
					}
	...
	protected void _retrieveWebAsset(ActionRequest req, ActionResponse res, PortletConfig config, ActionForm form,User user) throws Exception {
		...
		{			
			...
			if(InodeUtils.isSet(sibblingInode) && !sibblingInode.equals("0")){
				...
				Identifier identifier = IdentifierFactory.getIdentifierByInode(sibblingContentlet);
		...
		if (st == null || !InodeUtils.isSet(st.getInode())) {
			...
			if (InodeUtils.isSet(req.getParameter("selectedStructure"))) {
				...
				st = (Structure) InodeFactory.getParentOfClass(container, Structure.class);
	...
	throws Exception {
		...
		if (currentContentlet.getInode().equalsIgnoreCase(contentlet.getInode())) {
			...
			Identifier iden = IdentifierCache.getIdentifierFromIdentifierCache(contentlet);

			if (InodeUtils.isSet(htmlParent.getInode()) && InodeUtils.isSet(containerParent.getInode()) && InodeUtils.isSet(contentlet.getInode())) {
				Identifier htmlPageIdentifier = IdentifierCache.getIdentifierFromIdentifierCache(htmlParent);
				Identifier containerIdentifier = IdentifierCache.getIdentifierFromIdentifierCache(containerParent);
				Identifier contenletIdentifier = IdentifierCache.getIdentifierFromIdentifierCache(contentlet);
				...
				InodeFactory.saveInode(htmlParent);
	...
	throws Exception {
		...
		if (InodeUtils.isSet(req.getParameter("selectedStructure"))) {
			...
			String containerInode = req.getParameter("contentcontainer_inode");
			Container container = (Container) InodeFactory.getInode(containerInode, Container.class);
			structure = (Structure) InodeFactory.getParentOfClass(container, Structure.class);
			contentlet.setStructureInode(structure.getInode());
		}
				...
				if (field.getFieldType().equals(Field.FieldType.IMAGE.toString())
					...
					try {
						...
						if (InodeUtils.isSet(id.getInode())) {
							if (field.getFieldType().equals(Field.FieldType.IMAGE.toString())) {
								File inodeAux = (File) IdentifierFactory.getWorkingChildOfClass(id, File.class);
	...
	throws Exception {
		...
		if(perAPI.doesUserHavePermission(contentlet, PermissionAPI.PERMISSION_WRITE, user) && workingContentlet.isLocked()){
			...
			User userMod = null;
			try{
				userMod = APILocator.getUserAPI().loadUserById(workingContentlet.getModUser(),APILocator.getUserAPI().getSystemUser(),false);
			}catch(Exception nsu){
				Logger.info(this, "User not found using System User");
				userMod = APILocator.getUserAPI().getSystemUser(); 
			}
			if (!user.getUserId().equals(userMod.getUserId())) {
				Company comp = PublicCompanyFactory.getDefaultCompany();
				String message = LanguageUtil.get(comp.getCompanyId(), user.getLocale(), "message.contentlet.edit.locked");
		...
		try{
			if(perAPI.doesUserHavePermission(contentlet, PermissionAPI.PERMISSION_WRITE, user) 
					&& InodeUtils.isSet(workingContentlet.getInode()) 
					&& !populateaccept)//DOTCMS-6139
				conAPI.lock(workingContentlet, user, false);
		}catch (Exception e) {
			throw (new ActionException("Cannot lock asset", e));
		}
	...
	public void _saveWebAsset(ActionRequest req, ActionResponse res, PortletConfig config, ActionForm form, User user) throws Exception, DotContentletValidationException {
		...
		String subcmd = req.getParameter("subcmd");
		if ((subcmd != null) && subcmd.equals(com.dotmarketing.util.Constants.PUBLISH)) {
			Logger.debug(this, "Setting live to true so contentlet will publish on checkin");
			currentContentlet.setLive(true);
		}
	...
	public void downloadToExcel(HttpServletResponse response, User user, List contentletList) throws DotSecurityException{
		...
		if(contentletList.size() > 0) {
			...
			try {
				...
				List<Category> catTitles = getStructureCategories(st);
				for(Category cat : catTitles){
					pr.print(","+cat.getCategoryName());
				}
				...
				for(Contentlet content :  contentletsList2 ){
					...
					for (Field f : stFields) {
						...
						//http://jira.dotmarketing.net/browse/DOTCMS-3232	
						/*if(f.getFieldType().equals(Field.FieldType.HOST_OR_FOLDER.toString())){
							if(FolderAPI.SYSTEM_FOLDER_ID.equals(content.getFolder()))
					...
					for(Category catParent : catTitles){
						String catListTitle="";
						List<Category> children = catAPI.getChildren(catParent, user, false);
						if (children.size() >= 1 && catAPI.canUseCategory(catParent, user, false)) {
							//children = (List<Category>)CollectionUtils.retainAll(catList, children);
							for(Category cat : children){
								if(UtilMethods.isSet(cat.getKey())){
									catListTitle = catListTitle+","+cat.getKey();
								}else{
									catListTitle = catListTitle+","+cat.getCategoryName();
								}								
							}
							if(UtilMethods.isSet(catListTitle)){
								catListTitle="\""+catListTitle.substring(1)+"\"";
							}
						}

						if(catList.size() > 0){
							pr.print(","+catListTitle);
						} else if(catTitles.size() > 0){
							pr.print(",");
						}
					}
					pr.print("\r\n");
	...
	@SuppressWarnings("unchecked")
	public List<Category> getStructureCategories (Structure st) {
		Entity entity = EntityFactory.getEntity(st.getName());
	...
	public void _assignWebAsset(ActionRequest req, ActionResponse res,
			PortletConfig config, ActionForm form, User user, String formWebKey)
	throws Exception {

		String systemUserId = "System";
		ContentletForm cForm = (ContentletForm) form;

		// Getting the contentlets variables to work
		Contentlet cont = (Contentlet) req.getAttribute(WebKeys.CONTENTLET_EDIT);


		WorkflowTask task = new WorkflowTask();

		task.setTitle("Content: " + conAPI.getName(cont, user, false) + " needs to be reviewed/published.");

		String description = UtilMethods.isSet(cForm.getTaskComments())?cForm.getTaskComments():ContentletHTMLUtil.toPrettyHTMLString(cont);

		task.setDescription(description);

		String taskAssignment = cForm.getTaskAssignment();
		...
		hist.setChangeDescription("Review/Publish Content Task Creation");
		hist.setCreationDate(new Date());
		hist.setMadeBy(systemUserId);
		InodeFactory.saveInode(hist);
		task.addChild(hist);

		String comments = UtilMethods.isSet(cForm.getTaskComments())?cForm.getTaskComments():"Review/Publish Task Request";
		WorkflowsFactory.sendWorkflowChangeEmails(task, "A new Review/Publish Content Task was created", comments, user);
	}
	...
	private List searchContentlets(ActionRequest req, ActionResponse res, PortletConfig config, ActionForm form, User user, String from){
		try {
			...
			List<Map<String, String>> contentlets = contentletAjax.searchContentletsByUser(structureInode, listFieldsValues, listCategoriesValues, Boolean.parseBoolean(showDeleted), Boolean.parseBoolean(filterSystemHost), 0, currentSortBy, 100000, user, null, modDateFrom, modDateTo);
			return contentlets;
		} catch (Exception e) {
