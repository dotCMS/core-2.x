...
public class ContentletWebAPIImpl implements ContentletWebAPI {
	...
	public String saveContent(Map<String, Object> contentletFormData,
		...
		try {
			...
			_handleException(ae);
			throw new Exception(Constants.COMMON_ERROR);
		}
		...
		try {
			...
			try {
				String subcmd = (String) contentletFormData.get("subcmd");
				if ((subcmd != null) && subcmd.equals(com.dotmarketing.util.Constants.ASSIGNTO)) {
					Logger.debug(this, "Calling AssignTo Method");
					_assignWebAsset(contentletFormData, user, WebKeys.CONTENTLET_FORM_EDIT);
				}
			}catch(Exception e){
				if(!isAutoSave)
	...
	private void _assignWebAsset(Map<String, Object> contentletFormData,
			User user, String contentletFormEdit) throws Exception {
		...
		Contentlet cont = (Contentlet) contentletFormData.get(WebKeys.CONTENTLET_EDIT);
		
		WorkflowTask task = new WorkflowTask();
		task.setTitle("Content: " + conAPI.getName(cont, user, false) + " needs to be reviewed/published.");

		String description = (String) (UtilMethods.isSet(contentletFormData.get("taskComments"))?contentletFormData.get("taskComments"):ContentletHTMLUtil.toPrettyHTMLString(cont));
		task.setDescription(description);

		String taskAssignment = (String) contentletFormData.get("taskAssignment");
		...
		String comments = (String) (UtilMethods.isSet(contentletFormData.get("taskComments"))?contentletFormData.get("taskComments"):"Review/Publish Task Request");
		WorkflowsFactory.sendWorkflowChangeEmails(task, "A new Review/Publish Content Task was created", comments, user, request, response);	
	}
	...
	private void _saveWebAsset(Map<String, Object> contentletFormData,
		...
		for (Iterator iterator = contentletFormKeys.iterator(); iterator
				.hasNext();) {
			...
			if(elementName.equalsIgnoreCase("hostId") && 
				...
				currentContentlet.setFolder(FolderAPI.SYSTEM_FOLDER_ID);
			}
			if(elementName.equalsIgnoreCase("folderInode") && 
					InodeUtils.isSet(contentletFormData.get(elementName).toString())){
				String folderInode = contentletFormData.get(elementName).toString();
				Folder folder = fldrAPI.find(folderInode);
				if(!perAPI.doesUserHavePermission(folder,PermissionAPI.PERMISSION_CAN_ADD_CHILDREN, user, false)){
					SessionMessages.add(req, "message", "message.insufficient.permissions.to.save");
		...
		try{			
			...
			if(!isAutoSave){
				...
				currentContentlet = conAPI.checkin(currentContentlet, contRel,
													cats, perAPI.getPermissions(currentContentlet, false, true), user, false);
			}else{
