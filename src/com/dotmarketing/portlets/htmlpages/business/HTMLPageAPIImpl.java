...
public class HTMLPageAPIImpl extends BaseWebAssetAPI implements HTMLPageAPI {
	...
	public HTMLPage copy(HTMLPage source, Folder destination, boolean forceOverwrite, boolean copyTemplateContainers,
        ...
        List<Container> sourceContainers = templateAPI.getContainersFromTemplate(sourceTemplate, user, respectFrontendRoles);
        List<Container> copyContainers = templateAPI.getContainersFromTemplate(template, user, respectFrontendRoles);
	...
	public HTMLPage copy(HTMLPage source, Folder destination, boolean forceOverwrite, CopyMode copyMode,
		...
		newHTMLPage.setLive(source.isLive());
		...
		if (copyMode == HTMLPageAPI.CopyMode.USE_SOURCE_CONTENT) {
			...
			for (MultiTree multiTree : associatedSourceContentlets) {
				...
				if (contentletFolder != null) {
					Folder contentletDestFolder = folderAPI.createFolders(contentletFolder.getPath(), destinationHost);
		...
		if (isNew) {
			...
			TreeFactory.saveTree(new Tree(destination.getInode(), newHTMLPage.getInode()));

			// creates new identifier for this webasset and persists it
			com.dotmarketing.factories.IdentifierFactory.createNewIdentifier(newHTMLPage, destination);

			TreeFactory.saveTree(new Tree(destinationTemplate.getIdentifier(), newHTMLPage.getInode(), "parentPageTemplate", 0));
		} else {
	...
	public HTMLPage getWorkingHTMLPageByPageURL(String htmlPageURL, Folder folder) {
		List<HTMLPage> htmlPages = getChildrenClassByCondition(folder, HTMLPage.class, "working="
				+ DbConnectionFactory.getDBTrue() + " and page_url='" + htmlPageURL + "'");
	...
	protected String getCopyHTMLPageName(String htmlPageName, String fileExtension, Folder folder) {
		...
		List<HTMLPage> htmlPages = getChildrenClassByCondition(folder, HTMLPage.class, DbConnectionFactory.getDBTrue()
	...
	protected static Template getHTMLPageTemplate(Inode i) {
		Identifier templateIdent = (Identifier) InodeFactory.getParentOfClassByRelationType(i, Identifier.class,
				"parentPageTemplate");
		List<Template> templates;
		templates = TemplateFactory.getTemplateByCondition("deleted = "
				+ com.dotmarketing.db.DbConnectionFactory.getDBFalse() + "  and working = "
				+ DbConnectionFactory.getDBTrue() + " and identifier = '" + templateIdent.getInode() + "'");
		if (templates.size() > 1)
			Logger.error(HTMLPageFactory.class, "Page " + ((HTMLPage) i).getPageUrl()
					+ " has more than one template associated!!");
		if (templates.size() > 0)
	...
	public HTMLPage saveHTMLPage(HTMLPage newHtmlPage, Template template, Folder parentFolder, User user,
		...
		try {
			...
			if (pageExists) {
				...
				identifier = (Identifier) IdentifierFactory.getParentIdentifier(existingHTMLPage);
			}
			...
			Identifier testIdentifier = (Identifier) IdentifierFactory.getIdentifierByURI(newHtmlPage.getURI(parentFolder), host);
			...
			if (pageExists) {
				...
				newHtmlPage = (HTMLPage) saveAsset(newHtmlPage, identifier);
				...
				if (InodeUtils.isSet(parentFolder.getInode())
					...
					IdentifierCache.removeAssetFromIdCache(newHtmlPage);

					IdentifierCache.removeFromIdCacheByInode(newHtmlPage.getIdentifier());
					IdentifierCache.removeFromIdCacheByInode(newHtmlPage.getInode());
					IdentifierCache.removeFromIdCacheByInode(existingHTMLPage.getIdentifier());
					IdentifierCache.removeFromIdCacheByInode(existingHTMLPage.getInode());

					IdentifierFactory.updateIdentifierURI(newHtmlPage, parentFolder);
			...
			if (template != null) {
				...
				Identifier currentTemplateIdent = (Identifier) InodeFactory.getParentOfClassByRelationType(newHtmlPage,
						Identifier.class, "parentPageTemplate");

				// Adds template children from selected box
				Identifier templateIdentifier = IdentifierFactory.getParentIdentifier(template);

				if (currentTemplateIdent == null
						|| !InodeUtils.isSet(currentTemplateIdent.getInode())
						|| (InodeUtils.isSet(currentTemplateIdent.getInode()) && (!currentTemplateIdent.getInode()
								.equalsIgnoreCase(templateIdentifier.getInode())))) {
					// Updating the template that has changed
					for (Tree tree : TreeFactory.getTreesByChildAndRelationType(newHtmlPage, "parentPageTemplate")) {
						if (!tree.getParent().equalsIgnoreCase(currentTemplateIdent.getInode()))
							TreeFactory.deleteTree(tree);
					}
					Tree tree = TreeFactory.getTree(currentTemplateIdent.getInode(), newHtmlPage.getInode(),
							"parentPageTemplate");
					tree.setParent(templateIdentifier.getInode());
					tree.setChild(newHtmlPage.getInode());
			...
			DotHibernate.flush();
			DotHibernate.getSession().refresh(newHtmlPage);
	...
	public Template getTemplateForWorkingHTMLPage(HTMLPage htmlpage) {
		return htmlPageFactory.getHTMLPageTemplate(htmlpage, true);
	}
	...
	public List<HTMLPage> findLiveHTMLPages(Folder folder) {
		return findLiveChildren(folder, new HTMLPage());
	}
	...
	public List<HTMLPage> findWorkingHTMLPages(Folder folder) {
		return getChildrenClassByCondition(folder, HTMLPage.class, "working = "
	...
	public String getHTML(HTMLPage htmlPage) {
	...
	public String getHTML(HTMLPage htmlPage, boolean liveMode, String contentId) {
		return getHTML(htmlPage, liveMode, contentId, null);
	}
	//http://jira.dotmarketing.net/browse/DOTCMS-3392
	public String getHTML(HTMLPage htmlPage, boolean liveMode, String contentId, User user) {
		...
		String idInode = IdentifierCache.getPathFromIdCache(uri, host)
				.getInode();
        ...
        if(user==null){
			try {
				user = (com.liferay.portal.model.User) requestProxy.getSession()
		...
		try {
			...
			context = VelocityServlet.createContext(requestProxy, responseProxy);
			if(! liveMode ){
