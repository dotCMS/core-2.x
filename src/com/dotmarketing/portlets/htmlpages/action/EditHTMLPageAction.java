...
public class EditHTMLPageAction extends DotPortletAction implements
	...
	public void processAction(ActionMapping mapping, ActionForm form,
		...
		if ((cmd != null) && cmd.equals("newedit")) {
			try {
				...
				Folder folder = FolderFactory.createFolders(folderPath,hostId,req);
				...
				htmlForm.setSelectedparentPath(folder.getPath());
	...
	public void _editWebAsset(ActionRequest req, ActionResponse res,
		...
		hf.setSelectedparentPath(parentFolder.getPath());
	...
	public void _saveWebAsset(ActionRequest req, ActionResponse res,
		...
		Folder parentFolder = (Folder) InodeFactory.getInode(req.getParameter("parent"), Folder.class);
		//http://jira.dotmarketing.net/browse/DOTCMS-5899
		Identifier id = IdentifierFactory.getIdentifierByInode(existingHTMLPage);
		if(UtilMethods.isSet(id.getInode())){
			String URI = id.getURI();
			String uriPath = URI.substring(0,URI.lastIndexOf("/")+1);
			if(!uriPath.equals(parentFolder.getPath())){
				id.setURI(parentFolder.getPath()+existingHTMLPage.getPageUrl());
		...
		Identifier templateIdentifier = IdentifierCache.getIdentifierFromIdentifierCache(template);
		
		// Checking permissions
		_checkPermissions(existingHTMLPage, parentFolder, user, httpReq);

		// parent identifier for this file
		Identifier identifier = (Identifier) IdentifierFactory
				.getParentIdentifier(existingHTMLPage);
		...
		Identifier testIdentifier = (Identifier) IdentifierFactory
				.getIdentifierByURI(newHtmlPage.getURI(parentFolder), host);

		WebAsset working = (WebAsset) IdentifierFactory.getWorkingChildOfClass(testIdentifier, existingHTMLPage.getClass());
	
		// if this is a new htmlpage and there is already an identifier with
		// this uri AND there is a current working version, return
		if (!InodeUtils.isSet(existingHTMLPage.getInode())
				&& InodeUtils.isSet(testIdentifier.getInode()) && InodeUtils.isSet(working.getInode())) {
		...
		// Versioning
		if (InodeUtils.isSet(existingHTMLPage.getInode()) || (InodeUtils.isSet(testIdentifier.getInode()) && !InodeUtils.isSet(working.getInode()))) {
			
			//http://jira.dotmarketing.net/browse/DOTCMS-5770
			
			if(InodeUtils.isSet(testIdentifier.getInode()) && !InodeUtils.isSet(existingHTMLPage.getInode())){
				identifier = testIdentifier;
				WebAssetFactory.createAsset(newHtmlPage, user.getUserId(),parentFolder, identifier, true);
				DotHibernate.flush();
				workingAsset = newHtmlPage;
			}else{
				// Create the version asset
				...
				if (InodeUtils.isSet(parentFolder.getInode()) && !workingAsset.getURI(parentFolder).equals(identifier.getURI())) {
					...
					IdentifierFactory.updateIdentifierURI(workingAsset,parentFolder);
		...
		if (UtilMethods.isSet(template)) {
			...
			Identifier currentTemplateIdent = (Identifier) InodeFactory
					.getParentOfClassByRelationType(workingAsset,
	...
	public void _copyWebAsset(ActionRequest req, ActionResponse res,
		...
		if (parentInode != null && parentInode.length() != 0
			...
			parent = (Folder) InodeFactory.getParentOfClass(currentHTMLPage,Folder.class);
		    Logger.debug(this, "Parent Folder=" + parent.getInode());
		}
