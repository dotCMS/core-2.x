...
import com.dotmarketing.portlets.contentlet.util.HostUtils;
...
//                hostMap.put("childrenFoldersCount", getSubFoldersCount(host));
    ...
    public List<Map> openFolderTree (String parentInode) {
        ...
        Folder f = (Folder) InodeFactory.getInode(parentInode, Folder.class);
	...
	public List<Map<String, Object>> openFolderContent (String parentInode, String sortBy, boolean showArchived) {
	...
	public Map<String, Object> getFolderContent (String folderId, int offset, int maxResults, String filter, List<String> mimeTypes, 
			List<String> extensions, boolean showArchived, boolean noFolders, boolean onlyFiles, String sortBy, boolean sortByDesc) {
    	...
    	String condition = "(working = " + DbConnectionFactory.getDBTrue();
    	if (showArchived) {
    		condition += ")";
    	} else {
    		condition += " and deleted = " + DbConnectionFactory.getDBFalse() + ")";
    	}
        ...
        Folder f = (Folder) InodeFactory.getInode(folderId, Folder.class);
        
        if(!noFolders) {
        	List<Folder> folders = InodeFactory.getChildrenClassByCondition(f, Folder.class, "type='folder'");
        ...
        if(!onlyFiles) {
        	List<HTMLPage> pages = InodeFactory.getChildrenClassByCondition(f, HTMLPage.class, condition + " and type='htmlpage'");
        ...
        List<File> files = InodeFactory.getChildrenClassByCondition(f, File.class, condition + " and type='file_asset'");
        for (File file : files) {
        	List<Integer> permissions = new ArrayList<Integer>();
        	try {
        		permissions = permissionAPI.getPermissionIdsFromRoles(file, roles, usr);
        	} catch (DotDataException e) {
        		Logger.error(this, "Could not load permissions : ",e);
        	}
        	...
        	if(permissions.contains(PERMISSION_READ)){
        		...
        		fileMap.put("mimeType", servletContext.getMimeType(file.getFileName().toLowerCase()));
        		fileMap.put("name", file.getFileName());
        		fileMap.put("description", file.getFriendlyName());
        		fileMap.put("extension", FileUtil.getIconExtension(file.getExtension()));
        ...
        if(!onlyFiles) {
        	List<Link> links = InodeFactory.getChildrenClassByCondition(f, Link.class, condition+ " and type='links'");
        	for (Link link : links) {
        		java.util.List permissions = new ArrayList();
        		try {
	...
	private List<Map> getFoldersTree (Host host, Role[] roles) {
        ...
        List<Folder> children = folderAPI.findSubFolders(host);
	...
	private List<Map> getFoldersTree (Folder parent, Role[] roles) {
        FolderAPI folderAPI = APILocator.getFolderAPI();
        List<Folder> children = folderAPI.findSubFolders(parent);
    ...
    public Map<String, Object> renameFolder (String inode, String newName) {
    	...
    	Folder folder = (Folder) InodeFactory.getInode(inode, Folder.class);
    	result.put("lastName", folder.getName());
    	result.put("extension", "");
    	result.put("newName", newName);
    	result.put("inode", folder.getInode());
    	if (FolderFactory.renameFolder(folder, newName)) {
        	result.put("result", 0);
    	} else {
        	result.put("result", 1);
	...
	public boolean copyFolder(String inode, String newFolder) throws Exception {
		...
		if (!FolderFactory.existsFolder(newFolder)) {
			...
			FolderFactory.copyFolder(folder, parentHost);
		} else {
			Folder parentFolder = (Folder) InodeFactory.getInode(newFolder,
					Folder.class);
			...
			if (FolderFactory.isChildFolder(parentFolder, folder)) {
				//Trying to move a folder over one of its children
				return false;
			}

			FolderFactory.copyFolder(folder, parentFolder);
		}
		return true;
	}
	...
	public boolean moveFolder(String inode, String newFolder) throws Exception {
		...
		try {
			...
			if (!FolderFactory.existsFolder(newFolder)) {
				...
				if (!FolderFactory.moveFolder(folder, parentHost)) {
				...
				if (!FolderFactory.moveFolder(folder, parentFolder)) {
					//A folder with the same name already exists on the destination
					return false;
				}
			}
    ...
    public Map<String, Object> renameFile (String inode, String newName) throws Exception {
    	...
    	if (FileFactory.renameFile(file, newName, user)) {
        	result.put("result", 0);
    	} else {
        	result.put("result", 1);
        	if (file.isLocked())
    ...
    public String copyFile (String inode, String newFolder) throws Exception {
		...
		if (UtilMethods.isSet(file.getFileName()) && !FolderFactory.matchFilter(parent, file.getFileName())) 
    ...
    public boolean moveFile (String inode, String folder) throws Exception {
		...
		boolean ret = FileFactory.moveFile(file, parent);
		FileCache.removeFile(file);
		return ret;
	...
	public List<Map<String,Object>> getTreeMinInfoByHostId (String hostId) throws DotRuntimeException, PortalException, SystemException, DotDataException, DotSecurityException {
		...
		for (Host host : hosts) {
			...
			if(host.isArchived()==false){
				...
				List<Folder> subFolders = folderAPI.findSubFolders(host);
	...
	public List<Map<String, Object>> getFolderSubfolders(String parentFolderId) throws PortalException, SystemException, DotDataException, DotSecurityException {
		...
		FolderAPI folderAPI = APILocator.getFolderAPI();
		Folder parentFolder = folderAPI.find(parentFolderId);
		List<Folder> folders = folderAPI.findSubFolders(parentFolder);
		List<Map<String, Object>> foldersToReturn = new ArrayList<Map<String,Object>>(folders.size());
		for (Folder f: folders) {
			List permissions = new ArrayList();
			try {
	...
	public Map<String, Object> findHostFolder(String hostFolderId) throws PortalException, SystemException, DotDataException, DotSecurityException {
		try {
			if (InodeUtils.isSet(hostFolderId)) {
				...
				FolderAPI folderAPI = APILocator.getFolderAPI();
				Folder folder = folderAPI.find(hostFolderId);
				if(folder != null) {
