...
public class OrderMenuAction extends DotPortletAction {
	...
	throws Exception {
		...
		if (!path.equals("/")) {
			...
			java.util.List<Object> itemsList = FolderFactory.getMenuItems(folder);
			...
			java.util.List<Object> itemsList = FolderFactory.getFoldersByParentAndCondition(hostId,condition);
			h.put("menuItems", itemsList);
			userHasPublishPermission = _findPublishPermissionExists(itemsList);
	...
	private List _getHtmlTreeList(List items, boolean show, int depth) throws DotDataException{
		...
		l.add(FolderFactory.buildNavigationTree(items, depth, user));
