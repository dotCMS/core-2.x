...
DotPortletActionInterface {
	...
	public void _saveWebAsset(ActionRequest req, ActionResponse res,
		...
		Folder parent = (Folder) InodeFactory.getInode(req.getParameter("parent"), Folder.class);
		//http://jira.dotmarketing.net/browse/DOTCMS-5899
		Identifier id = IdentifierFactory.getIdentifierByInode(currentLink);
		if(UtilMethods.isSet(id.getInode())){
			String URI = id.getURI();
			String uriPath = URI.substring(0,URI.lastIndexOf("/")+1);
			if(!uriPath.equals(parent.getPath())){
				id.setURI(parent.getPath()+currentLink.getProtocal() + currentLink.getUrl());
		...
		if (InodeUtils.isSet(currentLink.getInode())) {
			Identifier identifier = IdentifierCache.getIdentifierFromIdentifierCache(currentLink);
			WebAssetFactory
		...
		List<Inode> parents = (List<Inode>) InodeFactory.getParentsOfClass(currentLink, Category.class);
		parents.addAll(InodeFactory.getParentsOfClass(currentLink, Identifier.class));
		parents.addAll(InodeFactory.getParentsOfClass(currentLink, Folder.class));
		...
		while (parentsIterator.hasNext()) {
			Inode parentInode = (Inode) parentsIterator.next();
...
//			}
			...
			if (!currentLink.isLive()) {
				if (!(parentInode instanceof Identifier)) {
					parentInode.deleteChild(currentLink);
				}
			}
		}
	...
	public void _copyWebAsset(ActionRequest req, ActionResponse res,
		...
		if (parentInode != null && parentInode.length() != 0
			...
			parent = (Folder) InodeFactory.getParentOfClass(currentLink,
					Folder.class);
			Logger.debug(this, "Parent Folder=" + parent.getInode());
		}
	...
	public void _getVersionBackWebAsset(ActionRequest req, ActionResponse res,
		...
		parents.addAll(InodeFactory.getParentsOfClass(linkVersion, Identifier.class));
		parents.addAll(InodeFactory.getParentsOfClass(linkVersion, Folder.class));
	...
	public void _moveWebAsset(ActionRequest req, ActionResponse res, PortletConfig config, ActionForm form, User user, Class webAssetClass,
		...
		if (parentInode != null && parentInode.length() != 0 && !parentInode.equalsIgnoreCase("")) 
		{
			...
			Folder oldParent = (Folder) InodeFactory.getParentOfClass(webAsset,Folder.class);
