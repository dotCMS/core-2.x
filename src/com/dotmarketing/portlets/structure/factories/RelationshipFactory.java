...
import com.dotmarketing.portlets.contentlet.business.ContentletFactoryDBImpl;
...
public class RelationshipFactory {
	...
	public static List<Relationship> getRelationshipsByParent (Structure parent) {
        ...
        DotHibernate dh = new DotHibernate(Relationship.class);
	...
	public static List<Tree> getAllRelationshipTrees(Relationship relationship, Contentlet contentlet, boolean hasParent) {
        ...
        Identifier iden = IdentifierFactory.getIdentifierByInode(contentlet);
        if (hasParent) {
    ...
    @SuppressWarnings("deprecation")
	public static List<Contentlet> getAllRelationshipRecords(Relationship relationship, Contentlet contentlet,
            boolean hasParent, boolean live, String orderBy) {
        List<Contentlet> matches = new ArrayList<Contentlet>();
        ...
        String iden = "";
        try{
        	iden = IdentifierCache.getIdentifierFromIdentifierCache(contentlet).getInode();
        ...
        if (hasParent) {
        	if (live)	
        		matches = getRelatedContentByParent(iden, relationship.getRelationTypeValue(),"contentlet.live = " + DbConnectionFactory.getDBTrue(), orderBy);
        	else 
        		matches = getRelatedContentByParent(iden, relationship.getRelationTypeValue(),"contentlet.working = " + DbConnectionFactory.getDBTrue(), orderBy);
        		
        	
        } else {
        	if (live)	
        		matches = getRelatedContentByChild(iden, relationship.getRelationTypeValue(),"contentlet.live = " + DbConnectionFactory.getDBTrue(), orderBy);
        	else 
        		matches = getRelatedContentByChild(iden, relationship.getRelationTypeValue(),"contentlet.working = " + DbConnectionFactory.getDBTrue(), orderBy);
        }
        return matches;
    }
    ...
    public static void saveRelationship(Relationship relationship) {
    	InodeFactory.saveInode(relationship);
    	CacheLocator.getRelationshipCache().removeRelationshipByInode(relationship);
    }

    // ### DELETE ###
    public static void deleteRelationship(String inode) {
    ...
    @SuppressWarnings("unchecked")
	public static List<Contentlet> getRelatedContentByParent(String parentInode, String relationType, String condition, String orderBy) {
        try {
        	
            DotHibernate dh = new DotHibernate(com.dotmarketing.portlets.contentlet.business.Contentlet.class);

            String sql = "SELECT {contentlet.*} from contentlet contentlet, inode contentlet_1_, tree tree1, tree tree2 "
            		+ "where tree1.parent = ? and tree1.relation_type = ? and tree1.child = tree2.parent "
                    + "and tree2.child = contentlet.inode "
                    + "and contentlet.inode = contentlet_1_.inode "
                    + "and " + condition;
            ...
            ContentletFactoryDBImpl conFac = new ContentletFactoryDBImpl();
    ...
    @SuppressWarnings("unchecked")
	public static List<Contentlet> getRelatedContentByChild(String childInode, String relationType, String condition, String orderBy) {
        try {
        	
            DotHibernate dh = new DotHibernate(com.dotmarketing.portlets.contentlet.business.Contentlet.class);

            String sql = "SELECT {contentlet.*} from contentlet contentlet, inode contentlet_1_, tree tree1, tree tree2 "
            		+ "where tree1.child = ? and tree1.relation_type = ? and tree1.parent = tree2.parent "
                    + "and tree2.child = contentlet.inode "
                    + "and contentlet.inode = contentlet_1_.inode "
                    + "and " + condition;
    ...
    public static List<Contentlet> getAllRelationshipRecords(Relationship relationship, Contentlet contentlet,
            boolean hasParent, boolean live) {
