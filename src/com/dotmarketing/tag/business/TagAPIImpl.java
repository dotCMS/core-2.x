...
public class TagAPIImpl implements TagAPI{
	...
	public Tag getTag(String name, String userId, String hostId) throws Exception {
        ...
        DotHibernate dh = new DotHibernate(List.class);
    	dh.setQuery("from tag in class com.dotmarketing.tag.model.Tag where lower(tagName) = ?");
        dh.setParam(name.toLowerCase());
        ...
        if (tags == null || tags.size() == 0) {
        	...
        	for(Tag tag : tags){
        		...
        		if(tag.getHostId().equals(host.getTagStorage())){
        			newTag = tag;
            		tagExists = true;
        		}
	    	}
	 ...
	 * @return the tagInode
	 */
	public TagInode getTagInode(String tagId, String inode) {
	 ...
	 */
	public void deleteTagInode(TagInode tagInode) {
                	...
                	try{
            			if(hostIdentifier.equals(newTagStorageId) && !newTagStorageId.equals(Host.SYSTEM_HOST)){
            				...
            				try{
            					...
            					Logger.error(this, "Tag with tag storage "+ newTagStorageId +"already exists");
            				}
                    	}
