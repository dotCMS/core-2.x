...
public class FixTask00012UpdateAssetsHosts implements FixTask {
	...
	private final static String selectInodesSQL = "select inode from identifier where host_inode is null or host_inode = '' or host_inode = '0'";
	...
	private final static String selectTemplateInodeByIdentifierSQL = "select template.inode from template, inode template_1_ where template_1_.identifier = ? and template.inode = template_1_.inode and template.working = " + DbConnectionFactory.getDBTrue();
	
	private final static String selectContainerInodeByIdentifierSQL = "select containers.inode from containers, inode containers_1_ where containers.inode = containers_1_.inode and containers_1_.identifier = ? and working = " + DbConnectionFactory.getDBTrue();
	
	private final static String selectLinkInodeByIdentifierSQL = "select links.inode from links, inode links_1_ where links_1_.identifier = ? and links_1_.inode = links.inode and links.working = " + DbConnectionFactory.getDBTrue();
	
	private final static String selectHTMLPageInodeByIdentifierSQL = "select htmlpage.inode from htmlpage, inode htmlpage_1_ where htmlpage_1_.identifier = ? and htmlpage.working = " + DbConnectionFactory.getDBTrue() + " and htmlpage_1_.inode = htmlpage.inode";
	
	private final static String selectFileInodeByIdentifierSQL = "select file_asset.* from file_asset, inode file_asset_1_ where file_asset_1_.identifier = ? and file_asset.working = " + DbConnectionFactory.getDBTrue() + " and file_asset_1_.inode = file_asset.inode";
	
	private final static String getHostIdQuery = "select inode.identifier " +
												 "from tree, " +
												 "     inode identifier, " +
												 "     contentlet, " +
												 "     structure, " +
												 ...
												 "      tree.parent=identifier.identifier and " +
												 "      identifier.inode=contentlet.inode and " +
	...
	private final static String updateIdentifierHost = "update identifier set host_inode=? where inode=?";
