...
//    	return new VelocityContext(ctx);
    ...
    public static boolean isUrlLive(String url, Host host) {
        return isUrlLive(url, host.getIdentifier());
    }

    public static boolean isUrlLive(String url, String hostId) {
        return (LiveCache.getPathFromCache(url, hostId) != null);
    }

    public static boolean isUrlPreview(String url, Host host) {
        return isUrlPreview(url, host.getIdentifier());
    }

    public static boolean isUrlPreview(String url, String hostId) {
    ...
    public static Folder getParentFolder(String childPath, Host host) {
        Folder childFolder = FolderFactory.getFolderByPath(childPath, host);
        return (Folder) InodeFactory.getParentOfClass(childFolder, Folder.class);
    }
    ...
    public static float getItemPriceWithDiscount(OrderItem orderItem, List<DiscountCode> discounts) {
        float totalDiscount = 0;
        for (DiscountCode discount : discounts) {
            if (DiscountCodeFactory._potentialDiscount(orderItem, discount)) {
                if (discount.getDiscountType() == 1) {
                    totalDiscount += (discount.getDiscountAmount() / 100) * orderItem.getItemPrice();
                } else {
                    totalDiscount += discount.getDiscountAmount();
                }
            }
        }
        return orderItem.getItemPrice() - totalDiscount;
    }

    public static float getItemPriceWithDiscount(ProductFormat productFormat, float itemPrice, List<DiscountCode> discounts) {
        float totalDiscount = 0;
        for (DiscountCode discount : discounts) {
            if (DiscountCodeFactory._potentialDiscount(productFormat, discount)) {
                if (discount.getDiscountType() == 1) {
                    totalDiscount += (discount.getDiscountAmount() / 100) * itemPrice;
                } else {
                    totalDiscount += discount.getDiscountAmount();
                }
            }
        }
        return itemPrice - totalDiscount;
    }

    public static float getItemPriceWithDiscount(OrderItemForm orderItemForm, List<DiscountCode> discounts) {
        float totalDiscount = 0;
        for (DiscountCode discount : discounts) {
            if (DiscountCodeFactory._potentialDiscount(orderItemForm, discount)) {
                if (discount.getDiscountType() == 1) {
                    totalDiscount += (discount.getDiscountAmount() / 100) * orderItemForm.getItemPrice();
                } else {
                    totalDiscount += discount.getDiscountAmount();
                }
            }
        }
        return orderItemForm.getItemPrice() - totalDiscount;
    }

    public static Product getProduct(String inode) {
        return (Product) ProductFactory.getProduct(inode);
    }
    ...
    public static List<DiscountCode> getDiscountsByOrder(OrderForm orderForm) {
    ...
    public static List<DiscountCode> getDiscountsByOrder(Order order) {
        List<DiscountCode> discounts = new ArrayList<DiscountCode>();
    ...
    public static OrderForm loadOrderForm(HttpServletRequest request) {
        ...
        try {
            String orderInode = request.getParameter("orderInode");
            // Order
            ...
            orderForm.setLastModDate(order.getLastModDate());
            // Billing
            orderForm.setBillingAddress1(order.getBillingAddress1());
            orderForm.setBillingAddress2(order.getBillingAddress2());
            orderForm.setBillingCity(order.getBillingCity());
            orderForm.setBillingState(order.getBillingState());
            orderForm.setBillingZip(order.getBillingZip());
            orderForm.setBillingCountry(order.getBillingCountry());
            orderForm.setBillingPhone(order.getBillingPhone());
            orderForm.setBillingFax(order.getBillingFax());
            orderForm.setBillingContactName(order.getBillingContactName());
            orderForm.setBillingContactPhone(order.getBillingContactPhone());
            orderForm.setBillingContactEmail(order.getBillingContactEmail());
            // Shipping
            orderForm.setShippingAddress1(order.getShippingAddress1());
            orderForm.setShippingAddress2(order.getShippingAddress2());
            orderForm.setShippingCity(order.getShippingCity());

            orderForm.setShippingState(order.getShippingState());
            orderForm.setShippingZip(order.getShippingZip());
            orderForm.setShippingCountry(order.getShippingCountry());
            orderForm.setShippingPhone(order.getShippingPhone());
            orderForm.setShippingFax(order.getShippingFax());
            // Payment
            orderForm.setOrderTotal(order.getOrderTotal());
            orderForm.setOrderShipping(order.getOrderShipping());
            orderForm.setOrderTax(order.getOrderTax());
            orderForm.setOrderDiscount(order.getOrderDiscount());
            orderForm.setOrderSubTotal(order.getOrderSubTotal());
            orderForm.setOrderSubTotalDiscount(order.getOrderSubTotal() - order.getOrderDiscount());
            orderForm.setDiscountCodes(order.getDiscountCodes());
            if (order.getPaymentType().trim().equals(Config.getStringProperty("ECOM_CREDIT_CARD"))) {
                // CC
                orderForm.setCardNumber(order.getCardNumber());
                orderForm.setPaymentType("cc");
            } else if (order.getPaymentType().trim().equals(Config.getStringProperty("ECOM_CHECK"))) {
                // Check
                orderForm.setCheckNumber(order.getCheckNumber());
                orderForm.setCheckBankName(order.getCheckBankName());
                orderForm.setPaymentType("ch");
            } else if (order.getPaymentType().trim().equals(Config.getStringProperty("ECOM_PURCHASE_ORDER"))) {
                // PO
            ...
            // OrderItems
            List<OrderItem> orderItems = (List<OrderItem>) OrderItemFactory.getAllOrderItemsByParentOrder(order);
            List<OrderItemForm> orderItemsForm = new ArrayList<OrderItemForm>();
            for (OrderItem orderItem : orderItems) {
                ProductFormat format = ProductFormatFactory.getProductFormat(orderItem.getProductInode());
                OrderItemForm orderItemForm = new OrderItemForm();
                orderItemForm.setInode(orderItem.getInode());
                orderItemForm.setItemPrice(orderItem.getItemPrice());
                orderItemForm.setItemQty(orderItem.getItemQty());
                orderItemForm.setOrderInode(orderItem.getOrderInode());
                orderItemForm.setProductInode(orderItem.getProductInode());
                orderItemsForm.add(orderItemForm);
                orderItemForm.setFormatName(format.getFormat());
            }
            orderForm.setOrderItemList(orderItemsForm);
        } catch (Exception ex) {
            Logger.debug(UtilMethods.class, ex.getMessage());
            Logger.info(UtilMethods.class,ex.getMessage());
        }
        return orderForm;
    }

    public static String getUserFullName(OrderForm orderForm) {
        String fullName = "";
        try {
            UserProxy userProxy = com.dotmarketing.business.APILocator.getUserProxyAPI().getUserProxy(orderForm.getUserInode(),APILocator.getUserAPI().getSystemUser(), false);
            User user = APILocator.getUserAPI().loadUserById(userProxy.getUserId(), APILocator.getUserAPI().getSystemUser(), false);
    ...
    public static String getUserCompanyName(HttpServletRequest request, OrderForm orderForm) {
        String companyName = "";
        if (request.getSession().getAttribute(WebKeys.CMS_USER) != null) {
            UserProxy userProxy;
			try {
				userProxy = com.dotmarketing.business.APILocator.getUserProxyAPI().getUserProxy(orderForm.getUserInode(),APILocator.getUserAPI().getSystemUser(), false);
			} catch (Exception e) {
            ...
            if (InodeUtils.isSet(organization.getInode())) {
                companyName = organization.getTitle();
            }
        }
        return companyName;
    }
    ...
    public static Product getProductbyProductFormaId(String productFormatInode) {
    ...
    @Deprecated
    public static Product getProductbyProductFormaId(long productFormatInode) throws Exception {
    	return getProductbyProductFormaId(String.valueOf(productFormatInode));
    }

    public static Product getProductById(String productInode) {
    ...
    public static Template getVelocityTemplate(String liveUrl) throws WebAssetException, DotDataException, DotSecurityException {
        ...
        try {
            ...
            HTMLPage htmlPage = (HTMLPage) IdentifierFactory.getLiveChildOfClass(InodeFactory.getInode(idStr, Identifier.class), HTMLPage.class);
