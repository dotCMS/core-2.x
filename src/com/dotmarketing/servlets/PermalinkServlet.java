...
public class PermalinkServlet extends HttpServlet {
	...
	throws ServletException, IOException {
		...
		try {
			iden = IdentifierCache.getIdentifierFromIdentifierCache(_inode);
		} catch (Exception e) {
		...
		if(UtilMethods.isSet(request.getParameter("blog_name")) || UtilMethods.isSet(request.getParameter("excerpt")) ||
			...
			if(contentlets.size() > 0)
			{
				try {
					...
					Identifier pageIden = IdentifierCache.getIdentifierFromIdentifierCache(_page);
					//in case it is the inode of the page
					if(!InodeUtils.isSet(pageIden.getInode())){
						pageIden = IdentifierCache.getIdentifierFromIdentifierCache((HTMLPage)InodeFactory.getInode(_page, HTMLPage.class));
					}
