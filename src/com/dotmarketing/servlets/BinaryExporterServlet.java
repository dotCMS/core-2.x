...
public class BinaryExporterServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			boolean isContent = isContent(uuid, byInode);
			...
			if(isContent){
				...
				if(byInode) {
					...
					content = contentAPI.findContentletByIdentifier(assetIdentifier, userWebAPI.isLoggedToFrontend(req), 0, user, respectFrontendRoles);
				...
				if(byInode) {
					dotFile = FileCache.getFileByInode(assetInode);
				...
				Identifier id = IdentifierCache.getIdentifierFromIdentifierCache(assetIdentifier);
				...
				if(assetInode != null){
					inputFile = new File(FileFactory.getRealAssetPath(assetInode, UtilMethods.getFileExtension(dotFile.getFileName())));
				}
			...
			String mimeType = FileFactory.getMimeType(data.getDataFile().getName());
	...
	private boolean isContent(String id, boolean byInode){
		...
		if(byInode){
			if(FileCache.loadFromCacheOnly(id) != null){
				return false;
			}
			try {
				Contentlet c =APILocator.getContentletAPI().find(id, APILocator.getUserAPI().getSystemUser(), true);
			...
			try {
				if(FileCache.getFileByInode(id) != null){
					return false;
				}
			...
			Identifier identifier = IdentifierCache.loadFromCacheOnly(id);
			if(identifier != null){
				return identifier.getURI().startsWith("content");
			}
			//second check content check from lucene
			try {
				String luceneQuery = "+identifier:" + id;
				List l  =APILocator.getContentletAPI().searchIndex(luceneQuery, 1, 0, null, APILocator.getUserAPI().getSystemUser(), false);
				if(l.size()>0){
					return true;
				}
			} catch (Exception e1) {
				Logger.debug(this.getClass(), e1.getMessage());
			}
			
			
			try {
				identifier = IdentifierCache.getIdentifierFromIdentifierCache(id);
				if(identifier != null && identifier.getInode() != null){
					return identifier.getURI().startsWith("content");
				}
			} catch (DotHibernateException e) {
