...
//		}else{
		...
		List<String> tIds = queryForAssetIds("template, identifier, inode ",Template.class.getCanonicalName(),"template.inode", "identifier.inode","inode.identifier = identifier.inode and inode.inode = template.inode " 
				+ (
						...
						(UtilMethods.isSet(hostQuery)? " AND (" + hostQuery + ")":"")
			      ) +" AND template.deleted = " + DbConnectionFactory.getDBFalse() + " AND template.working = " + DbConnectionFactory.getDBTrue(), columnsToOrderBy, offset, limit, permission, respectFrontEndPermissions, user);
