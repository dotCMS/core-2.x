...
public class NavigationWebAPI implements ViewTool {
	...
	private void getTrail(StringBuffer stringbuffer, String openPath, String fullPath, String imgPath, String crumbTitle, HttpServletRequest request) throws DotSecurityException, PortalException, SystemException, DotDataException {
		...
		if (openPath.equals(fullPath) && openPath.endsWith("." + Config.getStringProperty("VELOCITY_PAGE_EXTENSION"))) {
			...
			Folder folder = FolderCache.getFolderByPathAndHostId(openPath, host.getIdentifier());
			...
			stringbuffer.append("<li><a href=\"" + UtilMethods.encodeURIComponent(folder.getPath()) + "\">");
	...
	private StringBuffer getPageTrail(StringBuffer stringbuffer, String fullPath, String crumbTitle, HttpServletRequest request) throws PortalException, SystemException, DotDataException, DotSecurityException {
		...
		if (UtilMethods.isSet(crumbTitle)) {
			...
			if(InodeUtils.isSet(request.getParameter("id"))) {
				try {
					Identifier id = IdentifierCache.getIdentifierFromIdentifierCache(request.getParameter("id"));
					long languageId = 0;
					try{
			...
			if ((cont != null) && (conTitle != null)) {
				...
				String idInode = IdentifierCache.getPathFromIdCache(fullPath, host).getInode();
	...
	public String createMenu(String startFromPath, int numberOfLevels, HttpServletRequest request) throws PortalException, SystemException, DotDataException, DotSecurityException
	{
		...
		try {
			...
			java.util.List<Inode> itemsList = new ArrayList<Inode>();
			String folderPath = "";
			String fileName = "";
			...
			if ("/".equals(startFromPath)) {
				String condition = "show_on_menu = " + com.dotmarketing.db.DbConnectionFactory.getDBTrue();
				...
				Folder folder = FolderCache.getFolderByPathAndHostId(startFromPath, hostId);
				Logger.debug(NavigationWebAPI.class, "NavigationWebAPI :: StaticMenuBuilder folder=" + folder.getPath());
					...
					for (Inode itemChild : itemsList) {
						...
						if (itemChild instanceof Folder) {
							...
							if (((File) itemChild).isWorking() && !((File) itemChild).isDeleted()) {
								stringbuf.append("#set ($VTLSERVLET_DECODED_URI=\"$UtilMethods.decodeURL($VTLSERVLET_URI)\")\n");
								stringbuf.append("#if ($UtilMethods.inString($VTLSERVLET_DECODED_URI,\"" + startFromPath + ((File) itemChild).getFileName()
										+ "\"))\n");
								stringbuf.append("<li class=\"active\"><a "+styleClass+" href=\"" + UtilMethods.encodeURIComponent(folderPath + ((File) itemChild).getFileName()) + "\">");
								stringbuf.append((addSpans?"<span>":"") + UtilHTML.escapeHTMLSpecialChars(((File) itemChild).getTitle()) + (addSpans?"</span>":"") + "</a></li>\n");
								stringbuf.append("#else\n");
								stringbuf.append("<li><a "+styleClass+" href=\"" + UtilMethods.encodeURIComponent(folderPath + ((File) itemChild).getFileName()) + "\">");
								stringbuf.append((addSpans?"<span>":"") + UtilHTML.escapeHTMLSpecialChars(((File) itemChild).getTitle()) + (addSpans?"</span>":"") + "</a></li>\n");
								stringbuf.append("#end \n");
							}
						}
	...
	/**
	  ...
	  */
	@SuppressWarnings("unchecked")
	private StringBuffer buildSubFolderMenu(StringBuffer stringbuf, Folder thisFolder, int numberOfLevels, int currentLevel, boolean addSpans, boolean isFirstItem, String firstItemClass, boolean isLastItem, String lastItemClass, String menuIdPrefix) {

		stringbuf.append("#set ($VTLSERVLET_DECODED_URI=\"$UtilMethods.decodeURL($VTLSERVLET_URI)\")\n");
		stringbuf.append("#if ($UtilMethods.inString($VTLSERVLET_DECODED_URI,\"" + thisFolder.getPath() + "\") || ($UtilMethods.isSet($openAllLevels) && $openAllLevels == true))\n");
		...
		stringbuf.append("#end\n");
		// gets menu items for this folder
		java.util.List<Inode> itemsChildrenList2 = FolderFactory.getMenuItems(thisFolder);

		// do we have any children?
		boolean nextLevelItems = (itemsChildrenList2.size() > 0 && currentLevel < numberOfLevels);

		String folderChildPath = thisFolder.getPath().substring(0, thisFolder.getPath().length() - 1);
		...
		stringbuf.append(" href=\"" + UtilMethods.encodeURIComponent(thisFolder.getPath()) + "\">");
			...
			for (Inode childChild2 : itemsChildrenList2) {
				...
				if (childChild2 instanceof Folder) {
					...
					if (currentLevel <= numberOfLevels) {
						...
						stringbuf.append("<li><a href=\"" + UtilMethods.encodeURIComponent(folderChildChild2.getPath()) + "index."
					...
					if (((HTMLPage) childChild2).isWorking() && !((HTMLPage) childChild2).isDeleted()) {
        				stringbuf.append("#set ($VTLSERVLET_DECODED_URI=\"$UtilMethods.decodeURL($VTLSERVLET_URI)\")\n");
						stringbuf.append("#if ($VTLSERVLET_DECODED_URI != '" + thisFolder.getPath() + ((HTMLPage) childChild2).getPageUrl() + "')\n");
						stringbuf.append("<li><a "+styleClass+" href=\"" + UtilMethods.encodeURIComponent(thisFolder.getPath() + ((HTMLPage) childChild2).getPageUrl()) + "\">");
						stringbuf.append((addSpans?"<span>":"") + UtilHTML.escapeHTMLSpecialChars(((HTMLPage) childChild2).getTitle()) + (addSpans?"</span>":"") + "</a></li>\n");
						stringbuf.append("#else\n");
						stringbuf.append("<li class=\"active\"><a "+styleClass+" href=\"" + UtilMethods.encodeURIComponent(thisFolder.getPath() + ((HTMLPage) childChild2).getPageUrl()) + "\">");
						stringbuf.append((addSpans?"<span>":"") + UtilHTML.escapeHTMLSpecialChars(((HTMLPage) childChild2).getTitle()) + (addSpans?"</span>":"") + "</a></li>\n");
						stringbuf.append("#end \n");
					}
					...
					if (((File) childChild2).isWorking() && !((File) childChild2).isDeleted()) {
        				stringbuf.append("#set ($VTLSERVLET_DECODED_URI=\"$UtilMethods.decodeURL($VTLSERVLET_URI)\")\n");
						stringbuf.append("#if ($VTLSERVLET_DECODED_URI != '" + thisFolder.getPath() + ((File) childChild2).getFileName() + "')\n");
						stringbuf.append("<li><a "+styleClass+" href=\"" + UtilMethods.encodeURIComponent(thisFolder.getPath() + ((File) childChild2).getFileName()) + "\">");
						stringbuf.append((addSpans?"<span>":"") + UtilHTML.escapeHTMLSpecialChars(((File) childChild2).getTitle()) + (addSpans?"</span>":"") + "</a></li>\n");
						stringbuf.append("#else\n");
						stringbuf.append("<li class=\"active\"><a "+styleClass+" href=\"" + UtilMethods.encodeURIComponent(thisFolder.getPath() + ((File) childChild2).getFileName()) + "\">");
						stringbuf.append((addSpans?"<span>":"") + UtilHTML.escapeHTMLSpecialChars(((File) childChild2).getTitle()) + (addSpans?"</span>":"") + "</a></li>\n");
						stringbuf.append("#end \n");
					}
				}
			...
			if ("/".equals(startFromPath)) {
				...
				if (!file.exists() || file.length() == 0) {
					...
					for(int i=1; i < startFromLevel;i++){
						...
						for(Inode inode : itemsList){
							if (inode instanceof Folder) {
								itemsList2.addAll(FolderFactory.getMenuItems((Folder) inode,orderDirection));
							}
						}
				...
				Folder folder = FolderCache.getFolderByPathAndHostId(startFromPath, hostId);
				Logger.debug(StaticMenuBuilder.class, "StaticMenuBuilder folder=" + folder.getPath());
				...
				if (!file.exists()) {
					itemsList = FolderFactory.getMenuItems(folder,orderDirection);
					for(int i=1; i < startFromLevel;i++){
						java.util.List<Inode> itemsList2 = new ArrayList<Inode>();
						for(Inode inode : itemsList){
							if (inode instanceof Folder) {
			...
			if (fileExists) {
				...
				if (itemsList.size() > 0) {
					...
					stringbuf.append("#end \n");
						...
						Folder parent = FolderFactory.getFolderByPath(currentPath, hostId);
						if(InodeUtils.isSet(parent.getInode())) {
							String encodedPath = UtilMethods.encodeURIComponent(parent.getPath());
					...
					for (Inode itemChild : itemsList) {
						...
						if (itemChild instanceof Folder) {
							...
							if (((File) itemChild).isWorking() && !((File) itemChild).isDeleted()) {
								...
								stringbuf.append(UtilHTML.escapeHTMLSpecialChars(((File) itemChild).getTitle()) + "</a></li>\n");
	...
	private StringBuffer buildSubFolderSiteMapMenu(StringBuffer stringbuf, Folder thisFolder, int numberOfLevels, int currentLevel, int orderDirection, String menuIdPrefix) throws DotDataException, DotSecurityException {
		...
		stringbuf.append("\t<li id=\"" + menuIdPrefix +  thisFolder.getName() + "\">\n");
		// gets menu items for this folder
		java.util.List<Inode> itemsChildrenList2 = FolderFactory.getMenuItems(thisFolder,orderDirection);
		...
		Identifier id = IdentifierCache.getPathFromIdCache(thisFolder.getPath() + "index." + Config.getStringProperty("VELOCITY_PAGE_EXTENSION"), host);
		if(id != null && InodeUtils.isSet(id.getInode()))
			stringbuf.append("<a href=\"" + UtilMethods.encodeURIComponent(thisFolder.getPath()) + "\">");
		...
		if (currentLevel < numberOfLevels) {
			...
			for (Inode childChild2 : itemsChildrenList2) {
				if (childChild2 instanceof Folder) {
					...
					if (((HTMLPage) childChild2).isWorking() && !((HTMLPage) childChild2).isDeleted()) {
						stringbuf.append("<li><a href=\"" + UtilMethods.encodeURIComponent(thisFolder.getPath() + ((HTMLPage) childChild2).getPageUrl()) + "\">");
						stringbuf.append(UtilHTML.escapeHTMLSpecialChars(((HTMLPage) childChild2).getTitle()) + "</a></li>\n");

					}
				} else if (childChild2 instanceof File) {
					if (((File) childChild2).isWorking() && !((File) childChild2).isDeleted()) {
						stringbuf.append("<li><a href=\"" + UtilMethods.encodeURIComponent(thisFolder.getPath() + ((File) childChild2).getFileName()) + "\">");
						stringbuf.append(UtilHTML.escapeHTMLSpecialChars(((File) childChild2).getTitle()) + "</a></li>\n");
	...
	public List<HTMLPage> getPagesList(String folderPath, HttpServletRequest request) throws PortalException, SystemException, DotDataException, DotSecurityException {
		...
		Host host = WebAPILocator.getHostWebAPI().getCurrentHost(request);
		Folder thisFolder = FolderFactory.getFolderByPath(folderPath, host);
		List<Inode> itemsChildrenList = (List<Inode>)FolderFactory.getMenuItems(thisFolder);
				...
				if (itemsList.size() > 0) {
					...
					for (Inode itemChild : itemsList) {
						...
						if (itemChild instanceof Folder) {
							...
							stringbuf.append("#set ($_dummy  = $menuItem.put(\"path\", \"" + folderChild.getPath() + "\"))\n\n");
			...
			for (Inode childChild2 : itemsChildrenList2) {
				...
				if (childChild2 instanceof Folder) {
					...
					if (currentLevel <= numberOfLevels) {
						...
						stringbuf.append("#set ($_dummy  = $menuItem" + submenuName + ".put(\"path\", \"" + folderChildChild2.getPath() + "\"))\n\n");
						stringbuf.append(submenu + "\n");
					...
					if (htmlpage.isWorking() && !htmlpage.isDeleted()) {
						...
						stringbuf.append("#set ($_dummy  = $menuItem" + submenuName + ".put(\"path\", \"" + thisFolder.getPath() + "\"))\n");
					...
					File fileItem = (File) childChild2;
					if (fileItem.isWorking() && !fileItem.isDeleted()) {
        				stringbuf.append("#set ($menuItem" + submenuName + " = $contents.getEmptyMap())\n");
