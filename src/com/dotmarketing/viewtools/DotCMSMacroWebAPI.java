...
public class DotCMSMacroWebAPI implements ViewTool {
	...
	public List getfileRepository(String folderPath, String searchFolder,
		...
		if (user != null) {
			List rolesList = null;
			try {
				rolesList = com.dotmarketing.business.APILocator.getRoleAPI().loadRolesForUser(user.getUserId());
			} catch (DotDataException e) {
				Logger.error(DotCMSMacroWebAPI.class,e.getMessage(),e);
				return rolesList;
			}		
		...
		Folder folder = FolderCache.getFolderByPathAndHostId(folderPath, host.getIdentifier());

		if (Boolean.parseBoolean(searchFolder)) {
			return FolderFactory.getFoldersByParent(folder,user, true);
		} 
		else 
		{		
			...
			String condition = "deleted=" + dbFalse + " and  live= " + dbTrue + " order by file_asset.file_name asc";
			return FileFactory.getFilesByParentFolderPerRoleAndCondition(roles,folder, condition, user);
		}
	...
	{	
		...
		{		
			...
			{
				links = InodeFactory.getChildrenClass(folder,Link.class);
			}
			else
			{
	...
	public List getEvents(String categories, int limit, int startDayOffset,
		...
		return EventFactory.getPublicEvents(fromCal.getTime(), toCal.getTime(),
				"", categories.split(","), limit);
	 ...
	 */
	public List<HashMap<String, File>> getMediaGalleryFolderFiles(String folderPath, Host host) {
		return getMediaGalleryFolderFiles(folderPath, host.getIdentifier());
	}
	
	
	public List<HashMap<String, File>> getMediaGalleryFolderFiles(String folderPath, String hostId) {
        ...
        List<File> fileList = FileFactory.getFileChildrenByConditionAndOrder(folder, "deleted=" + com.dotmarketing.db.DbConnectionFactory.getDBFalse() + " and live= " + com.dotmarketing.db.DbConnectionFactory.getDBTrue(), "sort_order");
        List<HashMap<String, File>> result = new ArrayList<HashMap<String, File>>();
        HashMap<String, File> resultFile;
        List<File> noPhotoFile = new ArrayList<File>();
        int pos;
        File fileListFile;
        String videoPhotoFileName;
        
        for (File file: fileList) {
        	resultFile = new HashMap<String, File>();
	 ...
	 */
	@Deprecated
    public List<HashMap<String, File>> getMediaGalleryFolderFiles(String folderPath, long hostId) {
