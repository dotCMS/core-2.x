		...
		try {
			...
			for (HashMap<String, String> favorite : favorites) {
				...
				if (strTok.hasMoreElements() && (page_url.indexOf("pageTitle=") > -1)) {
					...
					Identifier idInode = IdentifierCache.getPathFromIdCache(page_url_clean, host);
					HTMLPage livePage = (HTMLPage) IdentifierFactory.getLiveChildOfClass(idInode, HTMLPage.class);
