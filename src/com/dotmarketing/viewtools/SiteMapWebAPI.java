...
public class SiteMapWebAPI implements ViewTool {
    ...
    public String getSiteMap(HttpServletRequest request) {
        ...
        try {
            ...
            java.util.List itemsChildrenList = FolderFactory.getMenuItems(host);
    ...
    private StringBuffer getEntries(List itemsChildrenList, Host parentHost, StringBuffer stringbuf, int level) {
        ...
        while (itemsChildrenListIter.hasNext()) {
            ...
            if (childItem instanceof Folder) {
                ...
                if (level == 0) {
                    ...
                    stringbuf.append("<B><a href=\"" + folderchildItem.getPath() + "\">" + folderchildItem.getTitle()
                            + "</a></B><ul>\n");
                }
                java.util.List itemsChildrenListFolder = FolderFactory.getMenuItems(folderchildItem);
            ...
            } else if (childItem instanceof File) {
                if (((File) childItem).isWorking() && !((File) childItem).isDeleted()) {
                    stringbuf.append("<li><a href=\"" + "/" + ((File) childItem).getFileName() + "\">\n");
                    stringbuf.append(UtilHTML.escapeHTMLSpecialChars(((File) childItem).getTitle()) + "</a></li>\n");
                }
            }
    ...
    private StringBuffer getEntries(List itemsChildrenList, Folder parentFolder, StringBuffer stringbuf, int level) {
        ...
        while (itemsChildrenListIter.hasNext()) {
            ...
            if (childItem instanceof Folder) {
                ...
                if (((HTMLPage) childItem).isWorking() && !((HTMLPage) childItem).isDeleted()) {
                    stringbuf.append("<li><a href=\"" + parentFolder.getPath() + ((HTMLPage) childItem).getPageUrl()
                            + "\">\n");
                    stringbuf.append(((HTMLPage) childItem).getTitle() + "</a></li>\n");
                }
            } else if (childItem instanceof File) {
                if (((File) childItem).isWorking() && !((File) childItem).isDeleted()) {
                    stringbuf.append("<li><a href=\"" + parentFolder.getPath() + ((File) childItem).getFileName()
                            + "\">\n");
                    stringbuf.append(((File) childItem).getTitle() + "</a></li>\n");
                }
            }
