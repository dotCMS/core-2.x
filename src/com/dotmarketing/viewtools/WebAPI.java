...
public class WebAPI implements ViewTool {
	...
	public List getRecentlyPublished() throws PortalException, SystemException, DotDataException, DotSecurityException {
		...
		db.setMaxRows(10);
		db
		.setSQL("select uri, mod_date, title from identifier, tree, htmlpage where identifier.inode = tree.parent and tree.child = htmlpage.inode and htmlpage.live = "
...
//			Velocity.mergeTemplate(parsePath, "UTF-8", vc, w);
			...
			CacheLocator.getCache("Velocity").clearCache();
			return false;
		}
		return true;
	}
...
//			boolean success = Velocity.evaluate(vc, sw, "RenderTool.eval()", vtl);
	...
	public String getContentInode(String parsePath) {
		...
		Identifier iden = (Identifier) InodeFactory.getInode(id, Identifier.class);
	...
	public String getAssetPath(String path, String host){
		try{
			...
			String realPath = FileFactory.getRealAssetPath(fileName, ext);
			return realPath;
		}catch (Exception e) {
			Logger.error(this, e.getMessage(),e);
			return null;
	...
	public String getAssetInode(String path, Host host){
		try{
			...
			File f = FileFactory.getFileByURI(path, host, false);
			return f.getInode();
		}catch (Exception e) {
			Logger.error(this, e.getMessage(),e);
			return null;
	...
	public static float getItemPriceWithDiscount(OrderItemForm orderItemForm, List<DiscountCode> discounts){
		return UtilMethods.getItemPriceWithDiscount(orderItemForm, discounts);
	}

	public static List<DiscountCode> getDiscountsByOrder(OrderForm orderForm){
		return UtilMethods.getDiscountsByOrder(orderForm);
	}

	public Inode getLiveFileAsset (Identifier id) {
		return (Inode) IdentifierFactory.getLiveChildOfClass(id, File.class);
	}
	...
	public Inode getLiveFileAsset (String identifierInode) {
		return getLiveFileAsset((Identifier)InodeFactory.getInode(identifierInode, Identifier.class));
	}
	...
	public Event getEvent(String inode)
	{
		Event event = EventFactory.getEvent(inode);
		return event;
	}

	public List getEventsInCalendarByRange(String from,String to,List<String> calendarCategories,List<String> eventCategories,String keyword)
	{
		int maxEvents = 0;
		return getEventsInCalendarByRange(from,to,calendarCategories,eventCategories,keyword,maxEvents);
	}

	public List getEventsInCalendarByRange(String from,String to,List<String> calendarCategories,List<String> eventCategories,String keyword,int maxEvents)
	{
		List returnValue = new ArrayList();
		try
		{
			String[] calendar = new String[0];
			if(calendarCategories != null)
			{
				calendar = new String[calendarCategories.size()];
				calendarCategories.toArray(calendar);
			}
			String[] event = new String[0];
			if(eventCategories != null)
			{
				event = new String[eventCategories.size()];
				eventCategories.toArray(event);
			}
			returnValue = getEventsInCalendarByRange(from,to,calendar,event,keyword,maxEvents);
		}
		catch(Exception ex)
		{
	...
	public List getEventsInCalendarByRange(String from,String to,String[] calendarCategories,String[] eventCategories,String keyword)
	{
		int maxEvents = 0;
		return getEventsInCalendarByRange(from,to,calendarCategories,eventCategories,keyword,maxEvents);
	}

	public List getEventsInCalendarByRange(String from,String to,String[] calendarCategories,String[] eventCategories,String keyword,int maxEvents)
	{
		try
		{
			if(calendarCategories != null)
			{
				calendarCategories = calendarCategories[0].split(",");
			}
			SimpleDateFormat sdf = new SimpleDateFormat("MM/dd/yyyy hh:mm:ss");

			Date fromDate = (UtilMethods.isSet(from)? sdf.parse(from + " 00:00:00") : null);
			Date toDate = (UtilMethods.isSet(to) ? sdf.parse(to + " 23:59:59") : null);

			List events = EventFactory.getPublicEventsByDateRangeYParent(fromDate,toDate,calendarCategories,eventCategories,keyword,maxEvents, user, true);
			return events;
		}
		catch(Exception ex)
		{
	...
	public String getIdentifierByURI(String URI) throws PortalException, SystemException, DotDataException, DotSecurityException{
		Host host = WebAPILocator.getHostWebAPI().getCurrentHost(request);
		Identifier id = IdentifierCache.getPathFromIdCache(URI, host);
		return id.getInode();
	}
