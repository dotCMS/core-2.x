...
public class CrumbTrailBuilder implements ViewTool {
	...
	public String createCrumbTrail(String pagePath, Host host) throws JspException, DotSecurityException
	{
	...
	private StringBuffer getTrail(Host host, StringBuffer stringbuffer, String openPath,int idxOf, String path, HashSet listItems) throws DotSecurityException {
		...
		if (idxOf!=-1) {
			...
			if (listItems.add(folder.getTitle())) {
				stringbuffer.append("<li><a href=\"" + folder.getPath() + "\">");
	...
	private StringBuffer getPageTrail(Host host, StringBuffer stringbuffer, String openPath,String pagePath, HashSet listItems) throws DotSecurityException {
		   return  getPageTrail (host.getIdentifier(), stringbuffer, openPath, pagePath,listItems);
	}

	private StringBuffer getPageTrail(String hostId, StringBuffer stringbuffer, String openPath,String pagePath, HashSet listItems) throws DotSecurityException {
	    ...
	    if (UtilMethods.isSet(request.getParameter("crumbTitle"))) {
    		...
    		Folder folder = FolderFactory.getFolderByPath(openPath, hostId);
    
    		Identifier identifier = IdentifierFactory.getIdentifierByURI(folder.getPath() + pagePath, hostId);
    		HTMLPage htmlpage = (HTMLPage) IdentifierFactory.getWorkingChildOfClass(identifier,HTMLPage.class);
