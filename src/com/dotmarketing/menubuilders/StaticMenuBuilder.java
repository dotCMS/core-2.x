...
public class StaticMenuBuilder implements ViewTool {
    ...
    private StringBuffer getMenuItems(StringBuffer stringbuf, Folder folderChildChild, boolean linkFolders, int numberOfLevels, int currentLevel, String imagePrefix, String imageSuffix) {
		...
		//gets menu items for this folder
		java.util.List itemsChildrenList2 = FolderFactory.getMenuItems(folderChildChild);
		...
		String folderChildPath = folderChildChild.getPath().substring(0, folderChildChild.getPath().length() - 1);
		folderChildPath = folderChildPath.substring(0, folderChildPath.lastIndexOf("/"));
		
		stringbuf.append("#set ($VTLSERVLET_DECODED_URI=\"$UtilMethods.decodeURL($VTLSERVLET_URI)\")\n");
		stringbuf.append("#if ($VTLSERVLET_DECODED_URI != '"  + folderChildPath + "' && $VTLSERVLET_DECODED_URI != '" + folderChildChild.getPath() + "index." + Config.getStringProperty("VELOCITY_PAGE_EXTENSION") + "')\n");
		stringbuf.append("<li id=\"" + folderChildChild.getName()+ "\">");
		
		HTMLPage page = HTMLPageFactory.getLiveHTMLPageByPath(folderChildChild.getPath() + "index." + Config.getStringProperty("VELOCITY_PAGE_EXTENSION"), host);
		if (!InodeUtils.isSet(page.getInode())) {
			if (linkFolders) stringbuf.append("<a href=\""+UtilMethods.encodeURIComponent(folderChildChild.getPath()) + "\">");
		} else { 
			if (linkFolders) stringbuf.append("<a href=\""+UtilMethods.encodeURIComponent(folderChildChild.getPath()) + "index." + Config.getStringProperty("VELOCITY_PAGE_EXTENSION")+"\">");
		}
		...
		if (nextLevelItems) {
			stringbuf.append("#set ($VTLSERVLET_DECODED_URI=\"$UtilMethods.decodeURL($VTLSERVLET_URI)\")\n");
	        stringbuf.append("#if ($UtilMethods.inString($VTLSERVLET_DECODED_URI,\""  +  folderChildChild.getPath() + "\"))\n");
			stringbuf.append("<ul>\n");
		}
		...
		while (itemsChildrenListIter2.hasNext()) {
			...
			if (childChild2 instanceof Folder) {
				...
				if (currentLevel <= numberOfLevels) {
					...
					if (linkFolders) {
						stringbuf.append("<li><a href=\"" + UtilMethods.encodeURIComponent(folderChildChild2.getPath()) + "index." + Config.getStringProperty("VELOCITY_PAGE_EXTENSION") + "\">\n");
				...
				if (((HTMLPage)childChild2).isWorking() && !((HTMLPage)childChild2).isDeleted()) {
					stringbuf.append("#set ($VTLSERVLET_DECODED_URI=\"$UtilMethods.decodeURL($VTLSERVLET_URI)\")\n");
					stringbuf.append("#if ($VTLSERVLET_DECODED_URI != '"  + folderChildChild.getPath() + ((HTMLPage)childChild2).getPageUrl() + "')\n");
					stringbuf.append("<li><a href=\"" + UtilMethods.encodeURIComponent(folderChildChild.getPath() + ((HTMLPage)childChild2).getPageUrl()) + "\">\n");
					stringbuf.append(((HTMLPage)childChild2).getTitle() + "</a></li>\n");
                    stringbuf.append("#else\n");
					stringbuf.append("<li class=\"active\"><a href=\"" + UtilMethods.encodeURIComponent(folderChildChild.getPath() + ((HTMLPage)childChild2).getPageUrl()) + "\">\n");
					stringbuf.append(((HTMLPage)childChild2).getTitle() + "</a></li>\n");
                    stringbuf.append("#end \n");
				}
			}
				...
				if (((File)childChild2).isWorking() && !((File)childChild2).isDeleted()) {
					stringbuf.append("#set ($VTLSERVLET_DECODED_URI=\"$UtilMethods.decodeURL($VTLSERVLET_URI)\")\n");
					stringbuf.append("#if ($VTLSERVLET_URI != '"  + folderChildChild.getPath() + ((File)childChild2).getFileName() + "')\n");
					stringbuf.append("<li><a href=\"" + UtilMethods.encodeURIComponent(folderChildChild.getPath() + ((File)childChild2).getFileName()) + "\">\n");
					stringbuf.append(((File)childChild2).getTitle() + "</a></li>\n");
                    stringbuf.append("#else\n");
					stringbuf.append("<li class=\"active\"><a href=\"" + UtilMethods.encodeURIComponent(folderChildChild.getPath() + ((File)childChild2).getFileName()) + "\">\n");
					stringbuf.append(((File)childChild2).getTitle() + "</a></li>\n");
                    stringbuf.append("#end \n");
				}
			}
