...
public class HorizontalMenuBuilder implements ViewTool {
	   ...
	   try {
	   	...
	   	if (path.equals("/")) {
			...
			try {
		   	...
		   	Logger.debug(HorizontalMenuBuilder.class, "HorizontalMenuBuilder folder=" + folder.getPath());
    ...
    private StringBuffer getMenuItems(StringBuffer stringbuf, Folder folderChildChild, boolean linkFolders, int numberOfLevels, int currentLevel, String imagePrefix, String imageSuffix) {
		...
		if (itemsChildrenListIter2.hasNext()) {
			...
			stringbuf.append("<li id=\"" + folderChildChild.getName()+ "\">");
			if (linkFolders) stringbuf.append("<a href=\"" + folderChildChild.getPath() + "index." + Config.getStringProperty("VELOCITY_PAGE_EXTENSION") + "\">"); 
		}
		...
		while (itemsChildrenListIter2.hasNext()) {
			...
			if (childChild2 instanceof Folder) {
				...
				if (currentLevel <= numberOfLevels) {
					...
					if (linkFolders) {
						stringbuf.append("<li><a href=\"" + folderChildChild2.getPath() + "index." + Config.getStringProperty("VELOCITY_PAGE_EXTENSION") + "\">\n");
				...
				if (((HTMLPage)childChild2).isWorking() && !((HTMLPage)childChild2).isDeleted()) {
					stringbuf.append("<li><a href=\"" + folderChildChild.getPath() + ((HTMLPage)childChild2).getPageUrl() + "\">\n");
				...
				if (((File)childChild2).isWorking() && !((File)childChild2).isDeleted()) {
					stringbuf.append("<li><a href=\"" + folderChildChild.getPath() + ((File)childChild2).getFileName() + "\">\n");
