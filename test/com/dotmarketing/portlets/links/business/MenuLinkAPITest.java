...
public class MenuLinkAPITest extends ServletTestCase {
	...
	public void testCopySameFolder() throws Exception {
		...
		try {
			...
			List<Link> allVersions = identifierAPI.findVersionsandLiveandWorkingChildrenOfClass(identifier, Link.class);
			assertTrue("Invalid number of version: " + allVersions.size(), (allVersions.size() == 1));
			
			List<Link> menuLinks = InodeFactory.getChildrenClassByCondition(testFolder1, Link.class, "working=" + DbConnectionFactory.getDBTrue());
