...
public class ContentletAPITest extends ServletTestCase {
	...
	protected void setUp() throws Exception {
		...
		newContentlets.add(createContentlet(textValue, longTextValue, dateValue, longValue, floatValue, booleanValue, testFile.getIdentifier(), testImage.getIdentifier(), binaryFile, FolderAPI.SYSTEM_FOLDER_ID, null, APILocator.getUserAPI().getSystemUser()));
		...
		createJUnitTestBinary();
		newContentlets.add(createContentlet(textLanguageValue, longTextValue, dateValue, longValue, floatValue, booleanValue, testFile.getIdentifier(), testImage.getIdentifier(), binaryFile, FolderAPI.SYSTEM_FOLDER_ID, language, APILocator.getUserAPI().getSystemUser()));
	}
	...
	private void createJUnitTestFiles() throws Exception {
		...
		folderAPI.save(testFolder);
		
		TreeFactory.saveTree(new Tree(testHost1.getIdentifier(), testFolder.getInode()));
	...
	private Contentlet createContentlet(String textValue, String longTextValue, Date dateValue, long longValue, float floatValue, boolean booleanValue, String fileValue, String imageValue, java.io.File binaryValue, String hostFolderValue, Language language, User user) throws DotDataException,
		...
		newCont.setFolder(FolderAPI.SYSTEM_FOLDER_ID);
	...
	private Relationship createRelationShip(Structure structure,boolean required)
	{
	...
	protected void tearDown() throws Exception 
	{
		...
		try { folderAPI.delete(testFolder); } catch (Exception e) { }
	...
	public void testAddWithNoRequiredRelationship() throws DotDataException, DotSecurityException, Exception
	{
		...
		//Create the new contentlet
		createJUnitTestBinary();
		Contentlet contentlet = createContentlet(textValue, longTextValue, dateValue, longValue, floatValue, booleanValue, testFile.getIdentifier(), testImage.getIdentifier(), new java.io.File(testBinaryFileName), FolderAPI.SYSTEM_FOLDER_ID, null, APILocator.getUserAPI().getSystemUser());
		//Unpublish the contenlet
	...
	public void testCleanField() throws DotSecurityException, DotDataException {
		...
		Structure jUnitTestStructure = StructureCache.getStructureByName(structureName);
		List<Field> fields = FieldsCache.getFieldsByStructureName(structureName);
		User user = ua.getSystemUser();
		Object value;
	...
	public void testCopy() throws Exception {
		...
		try {
			...
			assertTrue("Invalid \"folder\" in first copy content.", testContentCopy1.getFolder().equals(FolderAPI.SYSTEM_FOLDER_ID));
			...
			assertTrue("Invalid \"folder\" in second copy content.", testContentCopy2.getFolder().equals(FolderAPI.SYSTEM_FOLDER_ID));
	...
	public void testCopyAnotherFolder() throws Exception {
		...
		try {
			assertFalse("Invalid \"identifier\" in first copy content.", testContentCopy1.getIdentifier().equals(testContent.getIdentifier()));
			assertFalse("Invalid \"inode\" in first copy content.", testContentCopy1.getInode().equals(testContent.getInode()));
			assertFalse("Invalid \"folder\" in first copy content.", testContentCopy1.getFolder().equals(FolderAPI.SYSTEM_FOLDER_ID));
			assertEquals("Invalid \"structureInode\" in first copy content.", testContent.getStructureInode(), testContentCopy1.getStructureInode());
			...
			for (int pos = 0; pos < fields.size(); ++pos) {
				...
				if (pos == 0) {
					...
					if (folderId != null) {
						assertFalse("Invalid \"" + field.getVelocityVarName() + "\" in first copy content. Not a Folder.", folderAPI.find(folderId) == null);
					}
				} else {
					assertEquals("Invalid \"" + field.getVelocityVarName() + "\" in first copy content.", testContentMap.get(field.getVelocityVarName()), testContentCopyMap.get(field.getVelocityVarName()));
				}
			...
			assertFalse("Invalid \"identifier\" in second copy content.", testContentCopy2.getIdentifier().equals(testContent.getIdentifier()));
			assertFalse("Invalid \"inode\" in second copy content.", testContentCopy2.getInode().equals(testContent.getInode()));
			assertFalse("Invalid \"folder\" in second copy content.", testContentCopy2.getFolder().equals(FolderAPI.SYSTEM_FOLDER_ID));
			assertEquals("Invalid \"structureInode\" in second copy content.", testContent.getStructureInode(), testContentCopy2.getStructureInode());
