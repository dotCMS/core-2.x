...
public class FileAPITest extends ServletTestCase {
	...
	public void testCopyOverwrite() throws Exception {
		...
		try {
			...
			List<File> allVersions = identifierAPI.findVersionsandLiveandWorkingChildrenOfClass(identifier, File.class);
			assertTrue("Invalid number of version: " + allVersions.size(), (allVersions.size() == 2));
			
			List<File> files = InodeFactory.getChildrenClassByCondition(testFolder2, File.class, "working=" + DbConnectionFactory.getDBTrue());
	...
	public void testCopyNoOverwrite() throws Exception {
		...
		try {
			...
			allVersions = identifierAPI.findVersionsandLiveandWorkingChildrenOfClass(identifier, File.class);
