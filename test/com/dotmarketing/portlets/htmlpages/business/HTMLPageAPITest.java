...
public class HTMLPageAPITest extends ServletTestCase {
	...
	private void createJUnitTestHTMLPage() throws DotDataException, DotSecurityException {
		...
		testContentlet1.setFolder(FolderAPI.SYSTEM_FOLDER_ID);
		...
		testContentlet2.setFolder(FolderAPI.SYSTEM_FOLDER_ID);
		...
		folderAPI.save(testFolder1);
		
		TreeFactory.saveTree(new Tree(testHost.getIdentifier(), testFolder1.getInode()));
		...
		folderAPI.save(testFolder2);
		
		TreeFactory.saveTree(new Tree(testHost.getIdentifier(), testFolder2.getInode()));
	...
	private void deleteJUnitTestHTMLPage() throws DotDataException, DotSecurityException, Exception {
		...
		folderAPI.delete(testFolder1);
		folderAPI.delete(testFolder2);
		
		hostAPI.delete(testHost, user, false);
	}
	
	public void testFindWorkingHTMLPages () {
		HTMLPageAPI pageAPI = APILocator.getHTMLPageAPI();
		FolderAPI api = APILocator.getFolderAPI();
		Folder f = api.findFolderByPath("/calendar");
		List<HTMLPage> pages = pageAPI.findWorkingHTMLPages(f);
	...
	private boolean containsPageByURI (List<HTMLPage> pages, String uri) {
	...
	public void testCopy() throws Exception {
		...
		try {
			...
			Identifier tempIdentifier = (Identifier) InodeFactory.getParentOfClassByRelationType(testHTMLPageCopy1, Identifier.class, "parentPageTemplate");
			List<Template> templates = identifierAPI.findVersionsandLiveandWorkingChildrenOfClass(tempIdentifier, Template.class);
			...
			tempIdentifier = (Identifier) InodeFactory.getParentOfClassByRelationType(testHTMLPageCopy2, Identifier.class, "parentPageTemplate");
			templates = identifierAPI.findVersionsandLiveandWorkingChildrenOfClass(tempIdentifier, Template.class);
			tempTemplate = null;
			for (Template template: templates) {
				if (template.getIdentifier().equals(tempIdentifier.getInode()) && template.isWorking()) {
					tempTemplate = template;
			...
			tempIdentifier = (Identifier) InodeFactory.getParentOfClassByRelationType(testHTMLPageCopy3, Identifier.class, "parentPageTemplate");
			templates = identifierAPI.findVersionsandLiveandWorkingChildrenOfClass(tempIdentifier, Template.class);
			tempTemplate = null;
			for (Template template: templates) {
				if (template.getIdentifier().equals(tempIdentifier.getInode()) && template.isWorking()) {
			...
			List<HTMLPage> htmlPages = InodeFactory.getChildrenClassByCondition(testFolder2, HTMLPage.class, "working=" + DbConnectionFactory.getDBTrue());
	...
	public void testCopyOverwrite() throws Exception {
		...
		try {
			...
			List<HTMLPage> allVersions = identifierAPI.findVersionsandLiveandWorkingChildrenOfClass(identifier, HTMLPage.class);
