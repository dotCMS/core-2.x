	...
	<div id="admin-site-tools-div">
		<% if (signedIn) { %>
			...
			<c:if test="<%= APILocator.getRoleAPI().doesUserHaveRole(user, APILocator.getRoleAPI().loadRoleByKey(Role.LOGIN_AS)) && request.getSession().getAttribute(WebKeys.PRINCIPAL_USER_ID) == null %>">
				<a href="javascript: portal_showLoginAs()"><span class="loginAsIcon"></span><bean:message key="login-as" /></a>
			</c:if>
			
			<% if (request.getSession().getAttribute(WebKeys.PRINCIPAL_USER_ID) == null) { %>
		        <a href="javascript: portal_showMyAccount()"><span class="userIcon"></span><%=LanguageUtil.get(pageContext, "my-account")%></a> 
				<a  href="<%=CTX_PATH%>/portal<%=PortalUtil.getAuthorizedPath(request)%>/logout?referer=<%=CTX_PATH%>"><%=LanguageUtil.get(pageContext, "Logout")%> (<%=user.getFullName()%>)</a>
			<% } else { %>
