...
<script>
	...
	var occupiedPositions = new Array();
		...
		function showDotCMSSystemMessage(message, isError){
			...
			var hideFn = dojo.partial(hideDotCMSSystemMessage, messagesCount);
			var fadeOutFn = dojo.fadeOut({node: "systemMessages" + messagesCount, delay: 3000, duration: 2000, onEnd: hideFn }).play;
			
			var fadeIn = dojo.fadeIn({node: "systemMessages" + messagesCount, duration: 500, onEnd: fadeOutFn });
			fadeIn.play();
	
			hideMessagesHandler = setTimeout(hideFn,3000);
		...
		function hideDotCMSSystemMessage(messageId){
			...
			dojo.style(dojo.byId("systemMessagesWrapper" + messageId), {display: 'none'});		
