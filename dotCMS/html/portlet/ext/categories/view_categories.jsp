...
<script type="text/javascript">
	...
	dojo.addOnLoad(function() {
		...
		manageBreadCrumbs(null, '<%= LanguageUtil.get(pageContext, "Top-Level") %>');
	...
	function manageBreadCrumbs(inode, name) {
	...
	function prepareCrumbs(inode, name) {
		if(name=='<%= LanguageUtil.get(pageContext, "Top-Level") %>') {
		...
		currentCatName = name;
		var destroy = false;
		dojo.forEach(dojo.query("#ulNav >"), function(node, i) {
				if(node.id == "a_"+inode) {
					destroy = true;
					return;
				}
		...
		CategoryAjax.saveOrUpdateCategory(save, currentInodeOrIdentifier, name, velVar, key, keywords, {
			callback:function(result) {
				...
				case 0:
					...
					if(!save) {
						message = '';
						showDotCMSSystemMessage('<%= LanguageUtil.get(pageContext, "message.category.update") %>');
					}
					break;
				case 1:
	...
	<liferay:param name="box_title" value="<%= LanguageUtil.get(pageContext,\"view-categories\") %>" />
		<div id="mainTabContainer" dojoType="dijit.layout.TabContainer" dolayout="false" style="width: 99%; margin-left: auto; margin-right:auto;"  >
			...
			<div id="TabOne" dojoType="dijit.layout.ContentPane"  title="<%=LanguageUtil.get(pageContext, "children")%>" style="overflow: hidden; ">
				...
				<br/>
				<div id="catHolder" style="text-align: center; "></div>
		...
		<div style="padding:0 0 10px 0; border-bottom:1px solid #ccc;">
			<form id="addCatPropertiesForm" dojoType="dijit.form.Form">
				<dl>
					...
					<dd><input id="addCatName" type="text"  required="true" onblur="fillVelocityVarName(); " invalidMessage="Required." dojoType="dijit.form.ValidationTextBox" /></dd>
