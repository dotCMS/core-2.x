...
//gets parent folder
Folder folder = (Folder) folderAPI.find(file.getParent());
...
<!-- Basic Properties -->    
      <div id="fileBasicTab" dojoType="dijit.layout.ContentPane" onShow="showEditButtonsRow()" title="<%= LanguageUtil.get(pageContext, "Basic-Properties") %>">
			<dl>
				...
				<dd>
				<input type="text" dojoType="dijit.form.TextBox" style="width:250px" name="title"  id="titleField" value="<%= UtilMethods.isSet(file.getTitle()) ? file.getTitle() : "" %>" />
				...
				<%if(InodeUtils.isSet(file.getInode())){%>
					...
					<%if(com.dotmarketing.util.UtilMethods.isImage(identifier.getURI())){%>
						<dt><%= LanguageUtil.get(pageContext, "Edit Image") %>:</dt>
						<dd>
	...
	<div id="fileAdvancedTab" refreshOnShow="true" onShow="showEditButtonsRow()" preload="true"  dojoType="dijit.layout.ContentPane" title="<%= LanguageUtil.get(pageContext, "Advanced-Properties") %>">
		<dl>
			...
			<%if(InodeUtils.isSet(identifier.getInode())){%>
				...
				<dd><span style="color:gray"><%= identifier.getInode() %></span></dd>
			<%}%>
			 <%if(InodeUtils.isSet(file.getInode())){%>
				  <dt><%= LanguageUtil.get(pageContext, "Version") %>:</dt>
				  <dd><span style="color:gray"><%= file.getInode()%></span></dd>
			 <% } %>
			...
			<%
				com.dotmarketing.portlets.entities.model.Entity entity = com.dotmarketing.portlets.entities.factories.EntityFactory.getEntity("Files");
				...
				while (catsIter.hasNext()) {
					...
					java.util.Set children = new java.util.HashSet (com.dotmarketing.factories.InodeFactory.getChildrenClass(category,com.dotmarketing.portlets.categories.model.Category.class));
				if (children.size()>1) {
			%>
				<dt><%= category.getCategoryName() %></dt>
				<dd>
					<select name="categorySelect" size="<%=children.size()-1%>" multiple="true">
						<%= com.dotmarketing.util.UtilHTML.getSelectCategories(category,1,identifier, user, false) %>
					</select>
				</dd>
					<% } %>
			<% } %>
