...
<%
	...
	Iterator<Category> userCatIt = userCategories.iterator();

	if (displayCategories.equalsIgnoreCase("true")) {
%>
<tr>
	<td colspan="3">
		<br>
...
<%
		...
		while(userCatIt.hasNext()){
			com.dotmarketing.portlets.categories.model.Category cat = (com.dotmarketing.portlets.categories.model.Category) userCatIt.next();
			i++;
...
%>
				<td valign="top">
					<b><%= cat.getCategoryName() %></b><br>
	      		<% java.util.List childCats  = com.dotmarketing.factories.InodeFactory.getChildrenClass(cat, com.dotmarketing.portlets.categories.model.Category.class,"sort_order"); %>
	      		<% if(childCats!=null){ %>
	      			<select size="4" name="categories" id="<%="categories"+i %>" multiple style="width: 150px">
		      			<%java.util.Iterator childCatIter = childCats.iterator();%>
			      		<%while(childCatIter.hasNext()){%>
			      			<%com.dotmarketing.portlets.categories.model.Category childCatUnk = (com.dotmarketing.portlets.categories.model.Category) childCatIter.next();%>
			      			<option value="<%= childCatUnk.getInode() %>" <%= (form.isCategorySelected(String.valueOf(childCatUnk.getInode()))? "selected" : "") %>><%= childCatUnk.getCategoryName() %></option>
			      		<%}%>
	      			</select>
      			<%}%>
				</td>
