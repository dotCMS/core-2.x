...
<script language="javascript">
	...
	function deleteStructure(inode) {
		if (confirm('<%= UtilMethods.escapeSingleQuotes(LanguageUtil.get(pageContext, "message.structure.delete.structure.and.content")) %>')) {
	  	if (confirm('<%= UtilMethods.escapeSingleQuotes(LanguageUtil.get(pageContext, "message.structure.delete.really.structure.and.content")) %>')) {
	...
	<!-- START First Colum -->
		<div class="yui-u first">
			<dl>
		    	...
		    	String selectorValue = UtilMethods.isSet(folder) && !folder.equals(com.dotmarketing.portlets.folders.business.FolderAPI.SYSTEM_FOLDER_ID)?folder:host;
				%>
		...
		<% List fields = structure.getFields();
			...
			for (int i = 0; i < fields.size(); i++) {
				...
				<% if(!field.getFieldType().equals(Field.FieldType.HIDDEN.toString()) || fAPI.isElementConstant(field)){ %>
					<div class="divalt_1 dojoDndItem " style="white-space:no-wrap;" id="<%=field.getInode() %>">
						<span class="hiddenInodeField" style="display:none; width: 0px;"><%=inode%></span>
						...
						<div class="item_cell structureFieldLabelClass" style="width:150px;" onclick="clickFieldRow('<%=field.getInode() %>')">
							<%=field.getFieldName()%>
						</div>
						<div class="item_cell fieldTypeCell" style="width:100px;" onclick="clickFieldRow('<%=field.getInode() %>')">
							<%=field.getFieldType()%>
						</div>
						<div class="item_cell" style="width:120px;" onclick="clickFieldRow('<%=field.getInode() %>')">
							<%=(field.getVelocityVarName()!=null) ? field.getVelocityVarName() : "&nbsp;"%>
						</div>
						<div class="item_cell" style="width:100px;" onclick="clickFieldRow('<%=field.getInode() %>')">
							<%=(field.getFieldContentlet()!=null) ? field.getFieldContentlet() : "&nbsp;"%>
						</div>
						<div class="item_cell" style="width:70px;" onclick="clickFieldRow('<%=field.getInode() %>')">
...
<%if (!InodeUtils.isSet(structure.getInode())) {
  ...
  <%}
	...
	function editFieldVariables(fieldId){
		fieldVariablesAdmin.showFieldVariables(fieldId);
	}
</script>
