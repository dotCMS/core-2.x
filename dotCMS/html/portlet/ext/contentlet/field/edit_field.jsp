...
<%@page import="com.dotmarketing.cache.IdentifierCache"%>
	...
	<div class="fieldValue" id="<%=field.getVelocityVarName()%>_field">
		...
		%>
			<input type="text" name="<%=field.getFieldContentlet()%>" id="<%=field.getVelocityVarName()%>"
				<%=(isNumber) ? "dojoType='dijit.form.NumberTextBox' style='width:100px;'" : "dojoType='dijit.form.TextBox' style='width:400px'" %>
	    ...
	    <div style="padding:0 10px;margin:0 0 30px 0;width:400px;">
			<div style="float: left;padding-top: 5px;">
				<%= UtilMethods.escapeSingleQuotes(LanguageUtil.get(pageContext, "Dictionary-Terms")) %>:
				<input
...
<%
    ...
    if (field.getFieldType().equals(Field.FieldType.HOST_OR_FOLDER.toString())) {
    	...
    	String selectorValue = UtilMethods.isSet(folder) && !folder.equals(FolderAPI.SYSTEM_FOLDER_ID)?folder:host;
%>
...
%>
	<div style="width:600px;padding-bottom:40px;text-align: center;">
	    <textarea  <%= isReadOnly?"readonly=\"readonly\"":"" %>
	        class="editWYSIWYGField" rows="7"
	        name="<%=field.getFieldContentlet()%>"
	        id="<%=field.getVelocityVarName()%>"  style="width:100%; height:350px;font-family:monospace;clear:both;"><%=UtilMethods.htmlifyString(textValue)%></textarea>
   		<div style="padding:10px;white-space: nowrap;" align="center">
		    <div class="WYSIWYGControls" style="float:left;">
		    ...
		    <!--  http://jira.dotmarketing.net/browse/DOTCMS-3839  -->
		    <div class="dictionaryTermsControls" style="float:left;padding-left:30px;padding-right:30px;">
 ...
 %>
	 ...
	 <!--  display -->
	 		...
	 		<%if(UtilMethods.isImage(fileName)){%>
				...
				<%}else{ %>
						<div dojoType="dotcms.dijit.image.ImageEditor"
							...
							binaryFieldId="<%=field.getVelocityVarName()%>"
	 	...
	 	<% } %>
 			...
 			<div id="<%=field.getVelocityVarName()%>" name="<%=field.getFieldContentlet()%>" <%= UtilMethods.isSet(fileName)?"fileName=" + fileName:"" %>
 				onRemove="removeThumbnail('<%=field.getVelocityVarName()%>')" dojoType="dotcms.dijit.form.FileAjaxUploader">
 			</div>
								...
								function init<%=counter%>() {
									<% if(UtilMethods.isSet(categoriesList)) {
										 for (Category cat: categoriesList) {
											 ...
											 if(add) {
											 %>
												addCatFromField<%=counter%>("<%= cat.getInode() %>", "<%= cat.getCategoryName() %>");
											<%}
									 	}
									}
									%>
