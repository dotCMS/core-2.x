...
<%boolean canReindexContents = APILocator.getRoleAPI().doesUserHaveRole(user,APILocator.getRoleAPI().loadRoleByKey(Role.CMS_POWER_USER))|| com.dotmarketing.business.APILocator.getRoleAPI().doesUserHaveRole(user,com.dotmarketing.business.APILocator.getRoleAPI().loadCMSAdminRole());%>
    ...
    function fillResults(data) {
        ...
        headers = data[1];
        document.getElementById('allSearchedContentsInodes').value = data[2];
        ...
        for (var j = 0; j < data.length; j++) {
            ...
            try{
            var popup = getContentletPopUp(inode,'<%= CTX_PATH %>',
                inode,'',
                '<%=referer%>', live,
                working,
                deleted,
                ...
                publish,
                '<%=user.getUserId()%>');
            popupsiframe.innerHTML = popupsiframe.innerHTML + " " + popup;
    ...
    function titleCell (data,text, x) {
        ...
        if(publish == "1") {
            ...
            ref = ref + "<input dojoType=\"dijit.form.CheckBox\" type=\"checkbox\" name=\"publishInode\" id=\""
                    + checkId + "\" value=\"" + inode + "\" onClick=\"togglePublish();updateUnCheckedList("
                    + "'" + inode + "'" + "," + "'" +  checkId + "'" +  ");\" ";

                if((document.getElementById("fullCommand").value == "true")
                    && (document.getElementById("allSearchedContentsInodes").value.indexOf(inode) > -1)
        ...
        ref = ref + "<a  href=\"javascript: " + editRef + "\">";

        ref =  "<div class='titleCellDiv'>" + ref +text +"</div>";
        return ref;
    }
    ...
    function doSearch1 (page, sortBy) {
        ...
        if(isInodeSet(structureInode)){
            ...
            ContentletAjax.searchContentlets (structureInode, fieldsValues, categoriesValues, showDeleted, filterSystemHost, currentPage, currentSortBy, dateFrom, dateTo, fillResults);
        }
        ...
        for(i = 0;i< cbCount ;i++){
                ...
                if (showArchive) {
                	enableFields([
						...
						dijit.byId('deleteButton').setAttribute("disabled", false),
							dijit.byId('archiveReindexButton').setAttribute("disabled", false),
                    ]);
				} else {
                	enableFields([
					    ...
					    <% if(canReindexContents){ %>
							dijit.byId('reindexButton').setAttribute("disabled", false),
					 	<% } %>
                    ]);
				}
				break;
				...
				if (showArchive) {
                    disableFields([
                    	...
                    	<% if(canReindexContents){ %>
                    		dijit.byId("archiveReindexButton").setAttribute("disabled", true),
						<% } %>
                    ]);
				} else {
                	disableFields([
						...
						<% if(canReindexContents){ %>
							dijit.byId('reindexButton').setAttribute("disabled", true),
						<% } %>
                    ]);
				}
    ...
    function  resizeBrowser(){
        ...
        dojo.style(e, "height", viewport_height -260+ "px");
    }
