        ...
        items: [
            ...
            <%for(Structure s : structures){%>
                <%=(started) ? "," :""%>
                {
                    ...
                    name: "<%=s.getInode()%>",
                    label: "<%=(s.getStructureType() ==2) ? "<span class='gearIcon'></span>" : "<span class='contentIcon'></span>" %> <%=s.getName()%>",
                    textLabel: "<%=s.getName()%>"
                }
                <%started = true;%>
            <%}%>
                        ...
                        <dd>
                            ...
                            <script>
                                ...
                                function updateSelectBoxImage(myselect) {
                                    ...
                                    dojo.style(selField, "backgroundPosition", "5px 6px");
                                }
        ...
        <!-- START Buton Row -->
            <div class="buttonRow">
                <div id="archiveButtonDiv" style="display:none">
                    <button dojoType="dijit.form.Button" id="unArchiveButton" onClick="unArchiveSelectedContentlets()" iconClass="unarchiveIcon" disabled="true" >
                        <%= UtilMethods.escapeSingleQuotes(LanguageUtil.get(pageContext, "Un-Archive")) %>
                    </button>

                    <button dojoType="dijit.form.Button" id="deleteButton" onClick="deleteSelectedContentlets()" iconClass="deleteIcon" disabled="true">
                        <%= UtilMethods.escapeSingleQuotes(LanguageUtil.get(pageContext, "Delete"))%>
                    </button>

                    <% if(canReindexContentlets){ %>
                        <button dojoType="dijit.form.Button" id="archiveReindexButton" onClick="reindexSelectedContentlets()" iconClass="reindexIcon">
                ...
                <div id="unArchiveButtonDiv">
                    <button dojoType="dijit.form.Button" id="publishButton"  onClick="publishSelectedContentlets()" iconClass="publishIcon" disabled="true">
                        <%= UtilMethods.escapeSingleQuotes(LanguageUtil.get(pageContext, "Publish")) %>
                    </button>

                    <button dojoType="dijit.form.Button"  id="unPublishButton" onClick="unPublishSelectedContentlets()" iconClass="unpublishIcon" disabled="true">
                        <%= UtilMethods.escapeSingleQuotes(LanguageUtil.get(pageContext, "Unpublish")) %>
                    </button>

                    <button dojoType="dijit.form.Button" id="archiveButton" onClick="archiveSelectedContentlets()" iconClass="archiveIcon" disabled="true">
                        <%= UtilMethods.escapeSingleQuotes(LanguageUtil.get(pageContext, "Archive"))%>
                    </button>

                    <% if(canReindexContentlets){ %>
                        <button dojoType="dijit.form.Button" id="reindexButton" onClick="reindexSelectedContentlets()" iconClass="reindexIcon" disabled="true">
                            <%= UtilMethods.escapeSingleQuotes(LanguageUtil.get(pageContext, "Reindex")) %>
                        </button>
                    <% } %>
                </div>
...
<div class="messageZone" id="messageZone" style="display:none;">
    <div class="loadingIcon"><%= LanguageUtil.get(pageContext, "Loading")%>...</div>
</div>
