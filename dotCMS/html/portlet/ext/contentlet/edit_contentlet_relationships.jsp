	...
	<div class="yui-g portlet-toolbar">
		<div class="yui-u first">
            <b><%= (sortList.size()>1 && rel.isParentRequired() && !records.isHasParent()) ? "<span class=\"required\"></span>"  :
               ...
               <%=   rel.getRelationTypeValue() + ": " + targetStructure.getName() %></b> (<%=("yes".equals(isParent)) ? LanguageUtil.get(pageContext, "Parent"): LanguageUtil.get(pageContext, "Child") %>)		</div>
		...
		<table border="0" cellpadding="2" cellspacing="2" width="100%" class="listingTable" id="<%= relationJsName %>Table">
				<thead>
					<tr class="beta">
	...
	%>
		...
		<%
		 ...
		 <%
				...
				if (!hasListedFields) {
				    Identifier contid = IdentifierCache.getIdentifierFromIdentifierCache(cont);
		%>
		...
		%>
				...
				function <%= relationJsName %>CreateRow(item,hint){
					...
					<%
					if(langs.size() > 1) {
					%>	// displays language flag, if multiple languages exists.
					...
					%>
						...
						// displays the publish/unpublish/archive status of the content.
