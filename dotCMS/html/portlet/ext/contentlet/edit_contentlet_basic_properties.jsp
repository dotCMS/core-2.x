...
<%@page import="com.dotmarketing.factories.IdentifierFactory"%>
...
%>
    ...
    <html:hidden property="disabledWysiwyg" styleId="disabledWysiwyg" />
        ...
        <%if(InodeUtils.isSet(id.getInode()) && InodeUtils.isSet(contentlet.getInode())){%>
			<div class="fieldWrapper">	
                <div class="fieldName"><%= LanguageUtil.get(pageContext, "Identity") %>:</div>
                <div class="fieldValue fieldValueText"><%= id.getInode() %></div>
				<div class="clear"></div>
			</div>
        <%}%>
        <!-- SELECT STRUCTURE -->
            <div class="fieldWrapper">	
                <div class="fieldName">
		            <% if(structure.getStructureType() ==1){ %>
		                <%= LanguageUtil.get(pageContext, "Content-Type") %>:
		            <%}else if(structure.getStructureType() ==2){ %>
		
		                <%= LanguageUtil.get(pageContext, "Widget-Type") %>:
		            <%}else if(structure.getStructureType() ==3){ %>
		                <%= LanguageUtil.get(pageContext, "Form-Type") %>:
		            <%} %>
            	</div>
	            ...
	            <%} else {%>
					<div class="fieldValue fieldValueText">
        ...
        <%if (languages.size() > 1 && !structure.getVelocityVarName().equals("Host")) { %>
            ...
            <div class="fieldWrapper">	
                <div class="fieldName"><%= LanguageUtil.get(pageContext, "Language") %>:</div>
            	<div class="fieldValue">
      ...
      <% } %>
        ...
        <%if (UtilMethods.isSet(structure.getReviewerRole()) && structure.getStructureType() ==1){ %>
            <div class="fieldWrapper">
				<div class="fieldName"><%= LanguageUtil.get(pageContext, "Review") %>?:</div>
	            <div class="fieldValue">
	                ...
	                <span id="reviewContentDate" <%if(!contentletForm.isReviewContent()){ %> style="display: none;" <%} %>>
	                    <select dojoType="dijit.form.FilteringSelect" name="reviewIntervalNum"
	                        id="reviewIntervalNumId" style="width: 65px" 
	                        value="<%= UtilMethods.isSet(contentletForm.getReviewIntervalNum()) ? contentletForm.getReviewIntervalNum() : "" %>">
	                        <%  for (int i = 1; i <= 31; i++) {%>
	                            <option value='<%= "" + i %>'><%= "" + i %></option>
	                        <%}%>
	                    </select>
	                    <select dojoType="dijit.form.FilteringSelect" name="reviewIntervalSelect"
	                        id="reviewIntervalSelectId"  style="width: 110px" 
	                        value="<%= UtilMethods.isSet(contentletForm.getReviewIntervalSelect()) ? contentletForm.getReviewIntervalSelect() : "" %>" >
