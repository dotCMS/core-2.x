...
<div style="padding-left:20px;padding-bottom:10px;white-space: nowrap;font-size:88%;" class="permissionType">
	<%if(asset instanceof Folder){%>
		 <%= LanguageUtil.get(pageContext, "Folder") %>: <b><%= ((Folder) asset).getPath() %></b>
...
<!-- START Confirm Save permissions  change dialog -->	
     <div dojoType="dijit.Dialog" id="applyPermissionsChangesDialog" title='<%=LanguageUtil.get(pageContext, "Permissions-Confirmation") %>' style="display: none">
     	 <table align="center">
	        <tr><td colspan="2"><%=LanguageUtil.get(pageContext, "permissions-changes-confirmation") %></td></tr>
     		<tr>
     			<td> <button dojoType="dijit.form.Button" onClick="dijit.byId('applyPermissionsChangesDialog').hide(); applyPermissionChanges('${id}'); " type="button"><%= LanguageUtil.get(pageContext, "Yes") %></button> </td>
				<td> <button dojoType="dijit.form.Button" onClick="dijit.byId('applyPermissionsChangesDialog').hide();" type="button"><%= LanguageUtil.get(pageContext, "No") %></button> </td>	 
     		</tr>
		</table>				
     </div>
