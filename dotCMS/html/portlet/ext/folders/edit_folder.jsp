	...
	<div id="folderPropertiesTab" dojoType="dijit.layout.ContentPane" title="<%= LanguageUtil.get(pageContext, "Properties") %>" onShow="showEditButtonsRow()" >
		...
		<dl>
			<% if (InodeUtils.isSet(folder.getInode())) { %>
				...
				<dd><span style="color:gray"><%= folder.getInode() %></span></dd>
			<%}%>
			...
			<%if(InodeUtils.isSet(folder.getInode())){%>
				...
				<dd>
					<% if (parentFolder != null) { %>
						<%= parentFolder.getPath() %><%= folder.getName() %>
					<% } else { %>
						/<%= folder.getName() %>
					<% } %>
			...
			<dd>
				<input type="text" dojoType="dijit.form.TextBox"  onchange="beLazy();" style="width:250px" name="title"  id="friendlyNameField" value="<%= UtilMethods.isSet(folder.getTitle()) ? UtilMethods.escapeDoubleQuotes(folder.getTitle()) : "" %>" />
				
			</dd>
			
			<dt><span class="required"></span> <%= LanguageUtil.get(pageContext, "Name-URL") %>:</dt>
			<dd>
				<input type="text" dojoType="dijit.form.TextBox"   style="width:250px" name="name"  id="titleField" value="<%= UtilMethods.isSet(folder.getName()) ? UtilMethods.escapeDoubleQuotes(folder.getName()) : "" %>" />
			</dd>
			
			<dt><%= LanguageUtil.get(pageContext, "Sort-Order") %>:</dt>
			<dd>
				<input type="text" dojoType="dijit.form.TextBox"   style="width:60px" name="sortOrder"  value="<%= UtilMethods.isSet(folder.getSortOrder()+"") ? UtilMethods.escapeDoubleQuotes(folder.getSortOrder()+"") : "" %>" />
			
			</dd>
			
			<dt><%= LanguageUtil.get(pageContext, "Show-on-Menu") %>:</dt>
			<dd>
				<input type="checkbox" dojoType="dijit.form.CheckBox"  name="showOnMenu"  <%if(folder.isShowOnMenu()){ %> checked="checked" <% } %>/>
			</dd>
