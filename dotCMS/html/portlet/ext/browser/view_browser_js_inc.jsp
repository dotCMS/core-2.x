...
%>
	...
	//Dragging Events     
     ...
     function treeFolderSelected(inode) {
	    ...
	    if (isInodeSet(selectedFolder) && $(selectedFolder + '-TreeREF') != null) {
	 ...
	 function getStatusHTML (asset) {
		var html = 	'';
	 	...
	 	if (asset.locked)
			...
			html +=	'		<span id="' + asset.inode + '-StatusLockedIMG" class="unlockIcon"></span>\n';
		return html;
	 } 
   ...
   //Asset Actions
	...
	function deleteFolder(objId, referer) {
		if(confirm('<%= UtilMethods.escapeSingleQuotes(LanguageUtil.get(pageContext, "Are-you-sure-you-want-to-delete-this-folder-this-action-cant-be-undone")) %>')) {
			if (confirm("<%= UtilMethods.escapeSingleQuotes(LanguageUtil.get(pageContext, "The-selected-folder-is-not-empty-Are-you-sure-you-want-to-delete-it")) %>")){
	...
	//File actions
	function addFile(parentId,referer) {
		top.location = '<portlet:actionURL windowState="<%= WindowState.MAXIMIZED.toString() %>"><portlet:param name="struts_action" value="/ext/files/edit_file" /></portlet:actionURL>&cmd=edit&parent=' + parentId + '&inode=\'\'&referer=' + referer;
	}
	
	function addMultipleFile(parentId,referer) {
		top.location = '<portlet:actionURL windowState="<%= WindowState.MAXIMIZED.toString() %>"><portlet:param name="struts_action" value="/ext/files/upload_multiple" /></portlet:actionURL>&cmd=edit&parent=' + parentId + '&inode=\'\'&referer=' + referer;
	}
